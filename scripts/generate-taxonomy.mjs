import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OUTPUT_PATH = path.join(__dirname, '..', 'data', 'taxonomy.json');

const taxonomy = {
  cuisines: [
    {
      slug: 'moroccan',
      name: 'Moroccan Cuisine',
      country: 'Morocco',
      flag: '🇲🇦',
      description:
        'Moroccan cuisine is a vibrant tapestry of Berber, Arab, Andalusian, and Mediterranean influences, renowned for its bold use of spices like ras el hanout, saffron, and cumin. Slow-cooked tagines, fragrant couscous, and flaky pastilla define a culinary tradition that is as ceremonious as it is delicious.',
      categories: [
        {
          slug: 'moroccan-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'chicken-tagine',
              name: 'Chicken Tagine',
              description:
                'A slow-cooked Moroccan stew of tender chicken with preserved lemons, olives, and aromatic spices.',
              prepTime: '20 mins',
              cookTime: '1 hr 15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'lamb-tagine-with-prunes',
              name: 'Lamb Tagine with Prunes',
              description:
                'A rich, sweet-savory tagine of slow-braised lamb with dried prunes, almonds, and warming spices.',
              prepTime: '25 mins',
              cookTime: '2 hrs',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'beef-kefta',
              name: 'Beef Kefta',
              description:
                'Spiced ground beef blended with onion, parsley, and cumin, shaped and grilled on skewers or cooked in a tagine.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'couscous-royale',
              name: 'Couscous Royale',
              description:
                'The iconic Friday dish of fluffy couscous piled with seven seasonal vegetables and slow-cooked lamb.',
              prepTime: '30 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'bastilla',
              name: 'Bastilla',
              description:
                'A flaky warqa pastry pie filled with spiced pigeon or chicken, toasted almonds, eggs, and cinnamon sugar.',
              prepTime: '45 mins',
              cookTime: '45 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'merguez-with-couscous',
              name: 'Merguez with Couscous',
              description:
                'Spicy North African lamb and beef sausages served alongside buttery steamed couscous.',
              prepTime: '10 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'lamb-mechoui',
              name: 'Lamb Mechoui',
              description:
                'A whole slow-roasted lamb seasoned with cumin, paprika, and butter until the meat falls off the bone.',
              prepTime: '30 mins',
              cookTime: '5 hrs',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'fish-chermoula',
              name: 'Fish Chermoula',
              description:
                'Whole fish marinated in chermoula—a vibrant herb and spice sauce—then baked or grilled until tender.',
              prepTime: '20 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'rfissa',
              name: 'Rfissa',
              description:
                'A celebratory dish of shredded msemen flatbreads bathed in rich chicken and lentil fenugreek broth.',
              prepTime: '30 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'zaalouk',
              name: 'Zaalouk',
              description:
                'A smoky cooked salad of roasted eggplant and tomatoes seasoned with cumin, paprika, and olive oil.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'moroccan-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'harira',
              name: 'Harira',
              description:
                "Morocco's beloved tomato, lentil, and chickpea soup spiced with ginger and cinnamon, traditionally served to break the Ramadan fast.",
              prepTime: '20 mins',
              cookTime: '1 hr',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'bissara',
              name: 'Bissara',
              description:
                'A thick, velvety soup of dried split fava beans blended with olive oil, cumin, and paprika.',
              prepTime: '10 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chorba-frik',
              name: 'Chorba Frik',
              description:
                'A warming broth of lamb or chicken with cracked freekeh wheat and a bouquet of fresh herbs.',
              prepTime: '15 mins',
              cookTime: '50 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'moroccan-lentil-soup',
              name: 'Moroccan Lentil Soup',
              description:
                'A hearty red lentil soup seasoned with ras el hanout, turmeric, and a squeeze of lemon.',
              prepTime: '10 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'white-bean-soup',
              name: 'White Bean Soup',
              description:
                'Creamy white beans slowly simmered with tomatoes, garlic, and preserved lemon for a comforting bowl.',
              prepTime: '10 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chicken-vermicelli-soup',
              name: 'Chicken Vermicelli Soup',
              description:
                'A light golden chicken broth loaded with thin vermicelli noodles, saffron, and fragrant coriander.',
              prepTime: '15 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'lamb-chickpea-stew',
              name: 'Lamb and Chickpea Stew',
              description:
                'Braised lamb shoulder simmered with chickpeas, tomatoes, and a blend of warming Moroccan spices.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'moroccan-tomato-soup',
              name: 'Moroccan Tomato Soup',
              description:
                'A velvety roasted tomato soup brightened with chermoula spices and topped with a drizzle of argan oil.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'spiced-pumpkin-soup',
              name: 'Spiced Pumpkin Soup',
              description:
                'Silky pumpkin blended with ginger, cinnamon, and harissa for a warmly spiced Moroccan autumn soup.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'seafood-chorba',
              name: 'Seafood Chorba',
              description:
                'A rustic Moroccan-style seafood soup with shrimp, fish, and squid in a saffron-spiced tomato broth.',
              prepTime: '20 mins',
              cookTime: '35 mins',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'moroccan-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'msemen',
              name: 'Msemen',
              description:
                'Square-folded Moroccan flatbreads with a flaky, layered texture, best enjoyed warm with honey or argan oil.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'batbout',
              name: 'Batbout',
              description:
                'Soft pita-like Moroccan rolls cooked on a griddle, perfect for stuffing with kefta or cheese.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'sfenj',
              name: 'Sfenj',
              description:
                'Airy Moroccan ring doughnuts fried until golden and dusted with sugar, sold fresh at every morning market.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 10,
            },
            {
              slug: 'merguez-sandwich',
              name: 'Merguez Sandwich',
              description:
                'Sizzling spicy merguez sausages tucked into a crusty roll with harissa, tomatoes, and fresh herbs.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'harcha',
              name: 'Harcha',
              description:
                'Rustic semolina griddle cakes with a crumbly, buttery texture, sold at Moroccan street stalls every morning.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'maakouda',
              name: 'Maakouda',
              description:
                'Crispy fried potato fritters spiced with cumin and parsley, served in a sandwich or on their own.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'briouats',
              name: 'Briouats',
              description:
                'Crispy triangular filo pastry parcels filled with spiced minced meat or almonds and honey.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'bocadillo',
              name: 'Bocadillo',
              description:
                'A popular Moroccan street sandwich of canned sardines, harissa, and fresh vegetables in a crusty baguette.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'meloui',
              name: 'Meloui',
              description:
                'Spiral-rolled Moroccan flatbreads with a flaky, chewy crumb, enjoyed as a snack or with mint tea.',
              prepTime: '25 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'kaab-el-ghazal',
              name: 'Kaab el Ghazal',
              description:
                'Delicate crescent-shaped gazelle horn pastries filled with almond paste and orange blossom water.',
              prepTime: '45 mins',
              cookTime: '15 mins',
              difficulty: 'Hard',
              servings: 20,
            },
          ],
        },
        {
          slug: 'moroccan-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'chebakia',
              name: 'Chebakia',
              description:
                'Flower-shaped sesame and honey cookies fried and coated in aromatic honey, a staple of Moroccan celebrations.',
              prepTime: '40 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 30,
            },
            {
              slug: 'sellou',
              name: 'Sellou',
              description:
                'A dense, nutty no-bake Moroccan sweet made from toasted flour, sesame seeds, almonds, and honey.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 20,
            },
            {
              slug: 'gazelle-horns',
              name: 'Gazelle Horns',
              description:
                'Elegant crescent pastries with a tender crust encasing fragrant orange blossom and almond filling.',
              prepTime: '45 mins',
              cookTime: '15 mins',
              difficulty: 'Hard',
              servings: 20,
            },
            {
              slug: 'halwa-jeljlan',
              name: 'Halwa Dial Jeljlan',
              description:
                'A Moroccan sesame and honey confection pressed into bite-sized balls and rolled in sesame seeds.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 24,
            },
            {
              slug: 'orange-cinnamon-salad',
              name: 'Orange and Cinnamon Salad',
              description:
                'Thinly sliced fresh oranges dressed with cinnamon, orange blossom water, and a scatter of dates.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'bastilla-au-lait',
              name: 'Bastilla au Lait',
              description:
                'A creamy dessert bastilla filled with almond cream and milk custard in flaky warqa pastry layers.',
              prepTime: '30 mins',
              cookTime: '30 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'baghrir',
              name: 'Baghrir',
              description:
                'Thousand-hole semolina crepes with a spongy texture, drizzled with warm honey and melted butter.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'ghriba',
              name: 'Ghriba',
              description:
                'Crumbly almond or sesame shortbread cookies with a cracked sugar crust that melt in the mouth.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 20,
            },
            {
              slug: 'seffa',
              name: 'Seffa',
              description:
                'Steamed vermicelli or couscous dusted with powdered sugar, cinnamon, and fried almonds as a sweet course.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'shebakia-with-amlou',
              name: 'Shebakia with Amlou Dip',
              description:
                'Crispy fried sesame cookies served alongside amlou, a luscious dip of argan oil, almonds, and honey.',
              prepTime: '40 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 12,
            },
          ],
        },
        {
          slug: 'moroccan-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'beghrir',
              name: 'Beghrir',
              description:
                'Light and spongy thousand-hole pancakes made from semolina, best served warm with honey butter.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'msemen-with-honey',
              name: 'Msemen with Honey and Butter',
              description:
                'Warm, flaky square flatbreads drizzled with rich artisanal honey and fresh country butter.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'amlou-with-bread',
              name: 'Amlou with Bread',
              description:
                'A thick spreadable dip of roasted almonds, argan oil, and honey served alongside warm Moroccan bread.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'khobz',
              name: 'Khobz',
              description:
                'Round Moroccan hearth bread with a golden crust and chewy crumb, the cornerstone of every Moroccan breakfast.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'harcha-with-cheese',
              name: 'Harcha with Cream Cheese',
              description:
                'Golden semolina griddle cakes split open and filled with creamy cheese and a drizzle of honey.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'meloui-with-butter',
              name: 'Meloui with Butter and Jam',
              description:
                'Spiral Moroccan flatbreads served warm with salted butter and homemade fig or apricot jam.',
              prepTime: '25 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'ful-medames',
              name: 'Ful Medames',
              description:
                'Simmered fava beans seasoned with cumin, lemon, and olive oil, a protein-rich Moroccan morning staple.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'eggs-with-tomato-and-peppers',
              name: 'Eggs with Tomato and Peppers',
              description:
                'Eggs slow-cooked in a spiced tomato, onion, and sweet pepper sauce directly in a clay pan.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'lben-with-bread',
              name: 'Lben with Bread',
              description:
                'Tangy Moroccan buttermilk served chilled alongside warm khobz for a simple and refreshing breakfast.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'bissara-breakfast-bowl',
              name: 'Bissara Breakfast Bowl',
              description:
                'A warm bowl of blended fava bean soup topped with cumin, paprika, and a generous pour of olive oil.',
              prepTime: '10 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'moroccan-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'moroccan-mint-tea',
              name: 'Moroccan Mint Tea',
              description:
                'Ceremonial gunpowder green tea steeped with fresh spearmint and poured high for a characteristic frothy crown.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'avocado-smoothie',
              name: 'Avocado Smoothie',
              description:
                'Thick, creamy blended avocado with milk and honey, a beloved Moroccan café staple especially in Marrakech.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'moroccan-almond-milk',
              name: 'Moroccan Almond Milk',
              description:
                'Smooth chilled almond milk sweetened with sugar and perfumed with orange blossom water and rose water.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'rosewater-lemonade',
              name: 'Rosewater Lemonade',
              description:
                'Fresh-squeezed lemon juice sweetened with sugar syrup and delicately scented with rosewater and mint.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'bissap',
              name: 'Bissap',
              description:
                'A deep crimson herbal drink brewed from dried hibiscus flowers, sweetened and chilled over ice.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'tamarind-drink',
              name: 'Tamarind Drink',
              description:
                'A tangy-sweet cold beverage made from tamarind pulp dissolved in water and sweetened to balance its tartness.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'moroccan-ginger-tea',
              name: 'Moroccan Ginger Tea',
              description:
                'An invigorating hot brew of fresh ginger root, lemon, and honey with cinnamon and black pepper.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'fresh-orange-juice',
              name: 'Fresh Orange Juice',
              description:
                'Freshly squeezed Moroccan navel oranges, famously sweet and pressed to order at every Jemaa el-Fna stall.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'carob-drink',
              name: 'Carob Drink',
              description:
                'A warming, mildly sweet drink brewed from dried carob pods, traditionally consumed for its nutritional properties.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'leben',
              name: 'Leben',
              description:
                'A refreshing fermented milk drink similar to thin yogurt, enjoyed alongside savory Moroccan dishes or on its own.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
      ],
    },
    {
      slug: 'italian',
      name: 'Italian Cuisine',
      country: 'Italy',
      flag: '🇮🇹',
      description:
        'Italian cuisine is celebrated worldwide for its simplicity, quality ingredients, and deep regional diversity—from the rich ragùs of Bologna to the seafood-laden tables of Sicily. Built on the pillars of fresh pasta, olive oil, and seasonal produce, it remains one of the most beloved and influential food cultures in the world.',
      categories: [
        {
          slug: 'italian-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'spaghetti-carbonara',
              name: 'Spaghetti Carbonara',
              description:
                'A Roman classic of spaghetti tossed with guanciale, egg yolks, Pecorino Romano, and black pepper into a silky sauce.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'lasagna-bolognese',
              name: 'Lasagna Bolognese',
              description:
                'Layers of fresh pasta sheets alternating with slow-cooked meat ragù and béchamel sauce, baked until bubbling.',
              prepTime: '45 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'risotto-milanese',
              name: 'Risotto Milanese',
              description:
                'Creamy Arborio rice slowly stirred with beef bone marrow, white wine, and a generous pinch of saffron.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'osso-buco',
              name: 'Osso Buco',
              description:
                'Braised veal shanks slow-cooked in white wine and gremolata until the meat is fall-off-the-bone tender.',
              prepTime: '20 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'chicken-parmigiana',
              name: 'Chicken Parmigiana',
              description:
                'Breaded chicken breast fried golden then baked with tomato sauce and melted mozzarella in the Italian-American tradition.',
              prepTime: '25 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'saltimbocca',
              name: 'Saltimbocca alla Romana',
              description:
                'Thin veal cutlets wrapped in prosciutto and sage, pan-fried in butter and finished with a white wine glaze.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'bistecca-fiorentina',
              name: 'Bistecca Fiorentina',
              description:
                'A massive T-bone from Chianina cattle seasoned with salt only and cooked rare over blazing Tuscan oak coals.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'penne-arrabbiata',
              name: "Penne all'Arrabbiata",
              description:
                'Penne pasta in a fiery tomato sauce with garlic and dried chili peppers, garnished with fresh parsley.',
              prepTime: '5 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'branzino-al-forno',
              name: 'Branzino al Forno',
              description:
                'Whole sea bass baked with lemon, capers, white wine, and cherry tomatoes for a classic Italian coastal dish.',
              prepTime: '15 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'cacio-e-pepe',
              name: 'Cacio e Pepe',
              description:
                'A deceptively simple Roman pasta of tonnarelli tossed with Pecorino Romano and a bold crack of black pepper.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'italian-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'minestrone',
              name: 'Minestrone',
              description:
                'A hearty vegetable soup with seasonal produce, cannellini beans, and pasta in a rich tomato broth.',
              prepTime: '20 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'ribollita',
              name: 'Ribollita',
              description:
                'A thick Tuscan peasant soup of twice-boiled cannellini beans, cavolo nero, and stale bread for body.',
              prepTime: '20 mins',
              cookTime: '1 hr',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'pasta-e-fagioli',
              name: 'Pasta e Fagioli',
              description:
                'A creamy, rustic Italian soup of borlotti beans and short pasta in a thick seasoned broth.',
              prepTime: '15 mins',
              cookTime: '50 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'acquacotta',
              name: 'Acquacotta',
              description:
                "A Tuscan 'cooked water' soup of onions, celery, and tomatoes ladled over toasted bread with a poached egg.",
              prepTime: '15 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'zuppa-di-pesce',
              name: 'Zuppa di Pesce',
              description:
                'A luxurious Italian fish stew with a medley of seafood simmered in a white wine and saffron broth.',
              prepTime: '20 mins',
              cookTime: '35 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'italian-lentil-soup',
              name: 'Italian Lentil Soup',
              description:
                'Umbrian green lentils slowly simmered with pancetta, rosemary, and a Parmesan rind for depth.',
              prepTime: '10 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'stracciatella',
              name: 'Stracciatella',
              description:
                'A Roman egg-drop soup of clear chicken broth with beaten eggs, Parmigiano, and fresh nutmeg swirled in.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'zuppa-di-ceci',
              name: 'Zuppa di Ceci',
              description:
                'A warming Sicilian chickpea soup with garlic, rosemary, and a finishing drizzle of fruity olive oil.',
              prepTime: '10 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'cacciucco',
              name: 'Cacciucco',
              description:
                'A spicy Livornese seafood stew where five types of fish and shellfish cook down in tomatoes and red wine.',
              prepTime: '25 mins',
              cookTime: '1 hr',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'busecca',
              name: 'Busecca',
              description:
                'A Milanese tripe stew slow-cooked with beans, tomatoes, and vegetables in a deeply savory broth.',
              prepTime: '20 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
          ],
        },
        {
          slug: 'italian-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'arancini',
              name: 'Arancini',
              description:
                'Golden fried Sicilian rice balls stuffed with ragù, mozzarella, and peas in a crispy breadcrumb shell.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'suppli',
              name: 'Supplì',
              description:
                'Roman fried rice croquettes filled with gooey mozzarella and tomato sauce, crispier than their Sicilian cousin.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 10,
            },
            {
              slug: 'pizza-al-taglio',
              name: 'Pizza al Taglio',
              description:
                'Roman-style rectangular pizza baked in sheet pans with a thick, airy crust and endless topping combinations.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'piadina',
              name: 'Piadina Romagnola',
              description:
                'A thin flatbread from Romagna cooked on a griddle and filled with prosciutto, soft cheese, and arugula.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'focaccia',
              name: 'Focaccia Genovese',
              description:
                'Pillowy olive oil flatbread dimpled by hand and sprinkled with coarse sea salt and fresh rosemary.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'porchetta-sandwich',
              name: 'Porchetta Sandwich',
              description:
                'Thinly sliced herb-roasted pork belly and loin stuffed into a crusty roll drizzled with cooking juices.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'calzone',
              name: 'Calzone',
              description:
                'A folded pizza pocket stuffed with ricotta, salami, and mozzarella, baked or fried until golden and puffed.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'tigelle',
              name: 'Tigelle',
              description:
                'Small round Emilian bread discs cooked in a tigelliera and split open to fill with cured meats and cheese.',
              prepTime: '25 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'tramezzino',
              name: 'Tramezzino',
              description:
                'Triangular crustless white bread sandwiches layered with tuna, egg, or vegetables in classic Italian bar style.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'bombolone',
              name: 'Bombolone',
              description:
                'Fluffy Italian filled doughnuts piped full of vanilla custard cream and rolled in granulated sugar.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 10,
            },
          ],
        },
        {
          slug: 'italian-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'tiramisu',
              name: 'Tiramisù',
              description:
                'Classic Italian dessert of espresso-soaked ladyfingers layered with mascarpone cream and dusted with cocoa powder.',
              prepTime: '30 mins',
              cookTime: '0 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'cannoli',
              name: 'Cannoli Siciliani',
              description:
                "Crispy fried pastry tubes filled with sweet sheep's milk ricotta, chocolate chips, and candied orange peel.",
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'panna-cotta',
              name: 'Panna Cotta',
              description:
                'A silky set cream dessert from Piedmont, typically served with a fresh berry coulis or caramel sauce.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'sfogliatelle',
              name: 'Sfogliatelle',
              description:
                'Neapolitan shell-shaped pastries with hundreds of flaky layers filled with ricotta, semolina, and candied citrus.',
              prepTime: '60 mins',
              cookTime: '25 mins',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'zeppole',
              name: 'Zeppole di San Giuseppe',
              description:
                'Light choux pastry fritters deep-fried or baked and topped with custard cream and a glossy amarena cherry.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 10,
            },
            {
              slug: 'cantucci-biscotti',
              name: 'Cantucci Biscotti',
              description:
                'Twice-baked crunchy almond biscuits from Prato, traditionally dunked in Vin Santo dessert wine.',
              prepTime: '20 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 24,
            },
            {
              slug: 'torta-della-nonna',
              name: 'Torta della Nonna',
              description:
                "Grandmother's tart of tender shortcrust pastry filled with lemon pastry cream and topped with pine nuts.",
              prepTime: '30 mins',
              cookTime: '35 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'cassata',
              name: 'Cassata Siciliana',
              description:
                'A festive Sicilian sponge cake soaked in liqueur and filled with sweetened ricotta, marzipan, and candied fruits.',
              prepTime: '60 mins',
              cookTime: '30 mins',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'affogato',
              name: 'Affogato al Caffè',
              description:
                "A scoop of vanilla gelato 'drowned' in a shot of hot espresso, the perfect intersection of dessert and coffee.",
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'budino-di-riso',
              name: 'Budino di Riso',
              description:
                'Individual Florentine rice pudding cakes baked in shortcrust pastry shells with lemon zest and vanilla.',
              prepTime: '20 mins',
              cookTime: '35 mins',
              difficulty: 'Medium',
              servings: 8,
            },
          ],
        },
        {
          slug: 'italian-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'cornetto',
              name: 'Cornetto',
              description:
                "Italy's beloved crescent pastry, lighter and sweeter than a croissant, enjoyed plain or filled with jam or custard.",
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'cappuccino',
              name: 'Cappuccino with Brioche',
              description:
                'Velvety steamed milk and espresso in classic proportions, paired with a warm buttery Sicilian brioche col tuppo.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'fette-biscottate',
              name: 'Fette Biscottate with Jam',
              description:
                'Thin, crispy twice-baked bread slices spread with artisanal jam or honey, an everyday Italian breakfast staple.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'brioche-col-tuppo',
              name: 'Brioche col Tuppo',
              description:
                'A fluffy Sicilian brioche with its characteristic top-knot, best split open and filled with granita.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'ciambella',
              name: 'Ciambella',
              description:
                'A classic Italian ring cake flavored with lemon zest and vanilla, simple and light for the morning table.',
              prepTime: '20 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'crostata-di-marmellata',
              name: 'Crostata di Marmellata',
              description:
                'A rustic Italian lattice jam tart made with crumbly pasta frolla and filled with thick apricot or cherry jam.',
              prepTime: '20 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'bombolone-alla-crema',
              name: 'Bombolone alla Crema',
              description:
                'Pillowy sugar-coated fried doughnuts stuffed with silky pastry cream, a classic Italian bar breakfast.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'toast-al-prosciutto',
              name: 'Toast al Prosciutto e Formaggio',
              description:
                'A golden toasted sandwich of prosciutto cotto and melted cheese, a quick and satisfying Italian bar breakfast.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'yogurt-con-miele',
              name: 'Yogurt con Miele e Noci',
              description:
                'Creamy Italian yogurt topped with chestnut honey, toasted walnuts, and seasonal fresh fruit.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'macedonia-di-frutta',
              name: 'Macedonia di Frutta',
              description:
                'A refreshing medley of seasonal fresh fruits macerated in lemon juice and a little sugar.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'italian-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'espresso',
              name: 'Espresso',
              description:
                'A concentrated shot of coffee brewed under pressure, the cornerstone of Italian coffee culture and daily life.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'limoncello',
              name: 'Limoncello',
              description:
                'A vibrant lemon liqueur from the Amalfi Coast made by steeping lemon zests in pure spirit with sugar syrup.',
              prepTime: '20 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 20,
            },
            {
              slug: 'aperol-spritz',
              name: 'Aperol Spritz',
              description:
                "Italy's iconic aperitivo of Aperol, Prosecco, and a splash of soda water, served over ice with an orange slice.",
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'bellini',
              name: 'Bellini',
              description:
                "A Venetian cocktail of white peach purée and Prosecco, invented at Harry's Bar and beloved worldwide.",
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'negroni',
              name: 'Negroni',
              description:
                'A bittersweet Florentine cocktail of equal parts gin, Campari, and sweet vermouth, stirred over ice.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'prosecco',
              name: 'Prosecco',
              description:
                'Light, refreshing Italian sparkling wine from Veneto made with Glera grapes, ideal as an aperitivo.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'aranciata',
              name: 'Aranciata',
              description:
                'A sparkling Italian blood orange soda with a bittersweet citrus flavor, popular from Sicilian orange groves.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'caffe-macchiato',
              name: 'Caffè Macchiato',
              description:
                "An espresso shot 'stained' with a small dollop of warm foamed milk, a staple of Italian morning bars.",
              prepTime: '3 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'grappa',
              name: 'Grappa',
              description:
                'A potent Italian pomace brandy distilled from grape skins and seeds, traditionally sipped after a meal.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chinotto',
              name: 'Chinotto',
              description:
                'A dark, bittersweet Italian soft drink made from chinotto citrus fruit, popular as a non-alcoholic aperitivo.',
              prepTime: '3 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
      ],
    },
    {
      slug: 'japanese',
      name: 'Japanese Cuisine',
      country: 'Japan',
      flag: '🇯🇵',
      description:
        'Japanese cuisine is defined by its reverence for seasonal ingredients, precise technique, and the pursuit of umami. From the pristine simplicity of sushi and sashimi to the soul-warming depths of ramen broth, it encompasses a vast spectrum of flavors united by an aesthetic philosophy of balance, freshness, and visual harmony.',
      categories: [
        {
          slug: 'japanese-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'sushi-platter',
              name: 'Sushi Platter',
              description:
                'An assortment of nigiri and maki rolls crafted from vinegared rice, ultra-fresh fish, and seasonal toppings.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'tonkatsu',
              name: 'Tonkatsu',
              description:
                'Thick pork loin cutlets breaded in panko and deep-fried until golden, served with shredded cabbage and tonkatsu sauce.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'beef-teriyaki',
              name: 'Beef Teriyaki',
              description:
                'Tender slices of beef glazed in a sweet soy and mirin teriyaki sauce and grilled to a caramelised finish.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'yakitori',
              name: 'Yakitori',
              description:
                'Bite-sized pieces of chicken and vegetables threaded on bamboo skewers and grilled over charcoal with tare glaze.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tempura',
              name: 'Tempura',
              description:
                'Lightly battered and flash-fried prawns and seasonal vegetables served with a delicate dipping broth.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'chicken-karaage',
              name: 'Chicken Karaage',
              description:
                'Japanese fried chicken marinated in soy, ginger, and sake, double-fried for an irresistibly crispy crust.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'gyoza',
              name: 'Gyoza',
              description:
                'Pan-fried Japanese dumplings filled with pork and cabbage, crispy on the bottom and steamed to tender on top.',
              prepTime: '30 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'okonomiyaki',
              name: 'Okonomiyaki',
              description:
                'A savory Japanese pancake of shredded cabbage, batter, and your choice of fillings topped with mayo and bonito flakes.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'shabu-shabu',
              name: 'Shabu-Shabu',
              description:
                'Thinly sliced wagyu beef and vegetables swished briefly through a simmering kombu broth at the table.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'unagi-donburi',
              name: 'Unagi Donburi',
              description:
                'Grilled eel lacquered in a rich sweet soy glaze served over a bowl of fluffy steamed rice.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 2,
            },
          ],
        },
        {
          slug: 'japanese-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'miso-soup',
              name: 'Miso Soup',
              description:
                'A daily Japanese staple of dashi broth enriched with miso paste, silken tofu, wakame, and sliced spring onion.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tonkotsu-ramen',
              name: 'Tonkotsu Ramen',
              description:
                'Rich, milky pork bone broth ladled over springy noodles topped with chashu pork, soft-boiled egg, and nori.',
              prepTime: '30 mins',
              cookTime: '4 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'shoyu-ramen',
              name: 'Shoyu Ramen',
              description:
                'A clear, amber soy-seasoned chicken broth served with ramen noodles, bamboo shoots, and tender chashu.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'udon-soup',
              name: 'Udon Noodle Soup',
              description:
                'Thick, chewy wheat udon noodles in a golden dashi, soy, and mirin broth topped with tempura or fish cake.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'soba-soup',
              name: 'Soba Noodle Soup',
              description:
                'Earthy buckwheat soba noodles served hot in a subtly smoky tsuyu broth with nori and a sprinkle of wasabi.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'chawanmushi',
              name: 'Chawanmushi',
              description:
                'Silky steamed egg custard infused with dashi and concealing morsels of shrimp, chicken, and ginkgo nuts.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'oden',
              name: 'Oden',
              description:
                'A winter one-pot of daikon, fish cakes, boiled eggs, and konnyaku slowly simmered in a delicate dashi broth.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'tonjiru',
              name: 'Tonjiru',
              description:
                'A hearty miso soup loaded with pork belly, root vegetables, and burdock for a deeply warming Japanese meal.',
              prepTime: '15 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'ozoni',
              name: 'Ozoni',
              description:
                'A celebratory New Year soup of clear or miso broth with mochi rice cakes, vegetables, and seasonal garnishes.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'sukiyaki',
              name: 'Sukiyaki',
              description:
                'Thinly sliced beef and tofu simmered in a sweet soy and mirin broth, dipped in raw beaten egg before eating.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'japanese-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'takoyaki',
              name: 'Takoyaki',
              description:
                'Round Osaka-style balls of savory batter filled with diced octopus, drizzled with mayo and bonito flakes.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'taiyaki',
              name: 'Taiyaki',
              description:
                'Fish-shaped waffle cakes with a crisp outer shell filled with sweet red bean paste or custard cream.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'yakisoba',
              name: 'Yakisoba',
              description:
                'Stir-fried buckwheat noodles tossed with pork, cabbage, and a tangy Worcester-based sauce, topped with pickled ginger.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'onigiri',
              name: 'Onigiri',
              description:
                'Triangular or round seasoned rice balls wrapped in nori and filled with tuna mayo, salmon, or pickled plum.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'japanese-crepe',
              name: 'Japanese Harajuku Crepe',
              description:
                'A thin, freshly made crepe rolled into a cone and stuffed with whipped cream, fresh strawberries, and custard.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'ikayaki',
              name: 'Ikayaki',
              description:
                'Whole grilled squid basted with soy sauce on a skewer, a quintessential Japanese festival street snack.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'dango',
              name: 'Mitarashi Dango',
              description:
                'Chewy mochi rice dumplings on a skewer glazed with a sweet soy mitarashi sauce and lightly torched.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'senbei',
              name: 'Senbei',
              description:
                'Traditional Japanese rice crackers baked and brushed with soy sauce, sesame, or nori for a savory crunch.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 10,
            },
            {
              slug: 'melon-pan',
              name: 'Melon Pan',
              description:
                'Fluffy soft-centered Japanese sweet bread encased in a thin, crisp cookie crust with a distinctive grid pattern.',
              prepTime: '30 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'karaage-sandwich',
              name: 'Karaage Sandwich',
              description:
                'Crispy Japanese fried chicken nestled in a pillowy milk bread bun with shredded cabbage and Japanese mayo.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 2,
            },
          ],
        },
        {
          slug: 'japanese-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'mochi',
              name: 'Mochi Ice Cream',
              description:
                'Soft, pounded glutinous rice dough wrapped around a ball of ice cream in flavors like matcha, strawberry, and red bean.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'dorayaki',
              name: 'Dorayaki',
              description:
                'Two fluffy honey pancakes sandwiched together with a generous filling of sweet azuki red bean paste.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'matcha-ice-cream',
              name: 'Matcha Ice Cream',
              description:
                'Intensely flavored green tea ice cream made with ceremonial-grade matcha for a bittersweet, creamy finish.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'wagashi',
              name: 'Wagashi',
              description:
                'Artisan Japanese confections of bean paste and mochi shaped into seasonal motifs, served alongside matcha.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'anmitsu',
              name: 'Anmitsu',
              description:
                'A classic Japanese dessert bowl of agar jelly cubes, red bean paste, mochi, and fruit drizzled with kuromitsu syrup.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'kakigori',
              name: 'Kakigori',
              description:
                'Finely shaved ice mounded in a bowl and drenched with flavored syrups like matcha, strawberry, or condensed milk.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'japanese-cheesecake',
              name: 'Japanese Cotton Cheesecake',
              description:
                'An impossibly light, soufflé-textured cheesecake that jiggles like a cloud with a delicate cream cheese flavor.',
              prepTime: '30 mins',
              cookTime: '1 hr',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'castella',
              name: 'Castella',
              description:
                'A moist, honey-flavored sponge cake with Portuguese origins perfected over centuries in Nagasaki, Japan.',
              prepTime: '20 mins',
              cookTime: '50 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'yokan',
              name: 'Yokan',
              description:
                'A firm, elegant jelly of sweet red bean paste and agar set into blocks and sliced into refined portions.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 10,
            },
            {
              slug: 'crepe-cake',
              name: 'Mille Crêpe Cake',
              description:
                'Hundreds of paper-thin crepes layered with lightly sweetened whipped cream into a spectacular tower of texture.',
              prepTime: '60 mins',
              cookTime: '30 mins',
              difficulty: 'Hard',
              servings: 8,
            },
          ],
        },
        {
          slug: 'japanese-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'tamagoyaki',
              name: 'Tamagoyaki',
              description:
                'A rolled Japanese omelette seasoned with dashi, mirin, and soy, cooked layer by layer into a sweet, custardy log.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'natto-gohan',
              name: 'Natto Gohan',
              description:
                'Sticky fermented soybeans mixed with mustard and soy sauce served over steamed rice for a pungent, nourishing start.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'tamago-kake-gohan',
              name: 'Tamago Kake Gohan',
              description:
                'Hot steamed rice topped with a raw egg and a dash of soy sauce stirred together for Japan\'s simplest comfort breakfast.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'ochazuke',
              name: 'Ochazuke',
              description:
                'Leftover rice revived by pouring hot green tea or dashi over it, topped with pickles, nori, and sesame.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'okayu',
              name: 'Okayu',
              description:
                'Gentle Japanese rice porridge simmered until silky soft, topped with pickled plum, sesame, and spring onion.',
              prepTime: '5 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'grilled-salmon-breakfast',
              name: 'Grilled Salmon',
              description:
                'Salted salmon fillet grilled until the skin crisps, the cornerstone of a traditional Japanese breakfast set.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'onsen-tamago',
              name: 'Onsen Tamago',
              description:
                'Eggs slow-cooked in hot spring-temperature water until the white is barely set and the yolk remains silken.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'tsukemono',
              name: 'Tsukemono',
              description:
                'An array of Japanese pickled vegetables—cucumber, daikon, and plum—that cleanse the palate at breakfast.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tofu-breakfast-bowl',
              name: 'Tofu Breakfast Bowl',
              description:
                'Silken tofu served cold with katsuobushi, grated ginger, and a drizzle of soy sauce for a light morning meal.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'japanese-rice-set',
              name: 'Japanese Rice Set',
              description:
                'The classic ichiju-sansai breakfast of steamed rice, miso soup, grilled fish, pickles, and simmered vegetables.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 2,
            },
          ],
        },
        {
          slug: 'japanese-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'matcha',
              name: 'Matcha',
              description:
                'Ceremonial-grade powdered green tea whisked with hot water into a frothy, vibrant, bittersweet bowl.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'sake',
              name: 'Sake',
              description:
                'Japanese fermented rice wine with a clean, nuanced flavor enjoyed warm or chilled depending on the grade.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'japanese-green-tea',
              name: 'Japanese Green Tea',
              description:
                'Loose-leaf sencha or gyokuro steeped at precise temperatures to bring out a grassy, umami-rich sweetness.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'ramune',
              name: 'Ramune',
              description:
                'A beloved Japanese marble-bottle soda with a sweet, fizzy flavor in original or lychee and melon varieties.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'amazake',
              name: 'Amazake',
              description:
                'A sweet, low-alcohol or alcohol-free fermented rice drink with a warm, creamy consistency served at festivals.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'mugicha',
              name: 'Mugicha',
              description:
                'A refreshing roasted barley tea served cold in summer with a pleasantly bitter, toasty aroma.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'calpis',
              name: 'Calpis',
              description:
                'A lightly fermented milky soft drink with a subtle lactic tartness, diluted with water or soda and served chilled.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'umeshu',
              name: 'Umeshu',
              description:
                'Japanese plum wine made from unripe ume steeped in shochu and sugar, sweet, tart, and deeply fragrant.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'hojicha',
              name: 'Hojicha',
              description:
                'Roasted green tea with a warm, caramel-like fragrance and low caffeine, perfect as a calming evening drink.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'hokkaido-milk',
              name: 'Hokkaido Milk',
              description:
                'Creamy, rich whole milk from Hokkaido dairy farms, revered across Japan for its exceptional sweetness and body.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
          ],
        },
      ],
    },
    {
      slug: 'indian',
      name: 'Indian Cuisine',
      country: 'India',
      flag: '🇮🇳',
      description:
        'Indian cuisine is one of the world\'s most diverse and aromatic culinary traditions, shaped by thousands of years of history, trade, and regional variation. From the butter-enriched gravies of the north to the coconut and tamarind-laced curries of the south, it is a cuisine defined by complex spice blends, vibrant vegetables, and an extraordinary range of breads and rice dishes.',
      categories: [
        {
          slug: 'indian-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'butter-chicken',
              name: 'Butter Chicken',
              description:
                'Tandoor-roasted chicken simmered in a velvety tomato, cream, and butter sauce scented with fenugreek leaves.',
              prepTime: '20 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'chicken-biryani',
              name: 'Chicken Biryani',
              description:
                'Fragrant basmati rice layered with spiced chicken, saffron, caramelized onions, and sealed for the dum steam.',
              prepTime: '30 mins',
              cookTime: '1 hr',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'palak-paneer',
              name: 'Palak Paneer',
              description:
                'Fresh Indian cottage cheese cubes nestled in a vibrant, silky sauce of pureed spinach and aromatic spices.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'lamb-rogan-josh',
              name: 'Lamb Rogan Josh',
              description:
                'A Kashmiri slow-braised lamb curry with vivid red color from Kashmiri chilies, fennel, and dried ginger.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'dal-makhani',
              name: 'Dal Makhani',
              description:
                'Whole black lentils and kidney beans slow-cooked overnight with butter, cream, and tomatoes in a smoky tandoor.',
              prepTime: '15 mins',
              cookTime: '8 hrs',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'chole-bhature',
              name: 'Chole Bhature',
              description:
                'Spiced chickpea curry served with pillowy deep-fried leavened bread, a beloved Punjabi street meal.',
              prepTime: '20 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'fish-curry',
              name: 'Kerala Fish Curry',
              description:
                'Fresh fish fillets simmered in a tangy coconut milk gravy with kokum, green chilies, and curry leaves.',
              prepTime: '15 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tandoori-chicken',
              name: 'Tandoori Chicken',
              description:
                'Chicken marinated in yogurt and a bold spice blend, charred in a ripping-hot tandoor oven until smoky and tender.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'aloo-gobi',
              name: 'Aloo Gobi',
              description:
                'A dry-style vegetarian dish of potatoes and cauliflower pan-tossed with turmeric, cumin, and garam masala.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chicken-tikka-masala',
              name: 'Chicken Tikka Masala',
              description:
                'Charred chicken tikka pieces folded into a rich, spiced tomato and cream masala sauce.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'indian-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'dal-tadka',
              name: 'Dal Tadka',
              description:
                'Yellow toor lentils cooked until creamy, finished with a sizzling tarka of ghee, cumin, garlic, and dried chilies.',
              prepTime: '10 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'rasam',
              name: 'Rasam',
              description:
                'A thin, intensely peppery and tamarind-soured South Indian broth drunk as a digestive or served over rice.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'mulligatawny',
              name: 'Mulligatawny Soup',
              description:
                'An Anglo-Indian lentil and chicken soup spiced with curry powder, enriched with coconut milk and apple.',
              prepTime: '15 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'sambar',
              name: 'Sambar',
              description:
                'A South Indian toor lentil stew with seasonal vegetables and tamarind, tempered with mustard seeds and curry leaves.',
              prepTime: '15 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'shorba',
              name: 'Shorba',
              description:
                'An aromatic spiced broth of chicken or lamb simmered with ginger, coriander, and warming whole spices.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'kerala-fish-stew',
              name: 'Kerala Fish Stew',
              description:
                'A gently spiced coconut milk stew with fish, potato, and green chilies from the backwaters of Kerala.',
              prepTime: '15 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'yellow-dal-soup',
              name: 'Yellow Dal Soup',
              description:
                'A light, nourishing soup of yellow moong lentils seasoned with turmeric, lemon, and a tempering of ghee.',
              prepTime: '10 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tomato-shorba',
              name: 'Tomato Shorba',
              description:
                'A velvety roasted tomato soup scented with cardamom and black pepper, garnished with cream and coriander.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'paya-soup',
              name: 'Paya Soup',
              description:
                'Slow-cooked lamb trotters in a deeply gelatinous broth spiced with ginger, garlic, and warming whole spices.',
              prepTime: '20 mins',
              cookTime: '4 hrs',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'rajma',
              name: 'Rajma',
              description:
                'Tender red kidney beans cooked in a thick onion-tomato masala, a North Indian staple served over rice.',
              prepTime: '10 mins',
              cookTime: '1 hr',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'indian-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'pani-puri',
              name: 'Pani Puri',
              description:
                'Hollow, crispy semolina spheres filled with spiced potato and dunked in tangy tamarind and mint water.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'vada-pav',
              name: 'Vada Pav',
              description:
                "Mumbai's beloved street burger of spiced mashed potato fritter tucked into a soft pav bun with chutneys.",
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'pav-bhaji',
              name: 'Pav Bhaji',
              description:
                'A buttery spiced vegetable mash served with soft bread rolls griddled in a generous slick of butter.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'samosa',
              name: 'Samosa',
              description:
                'Crispy fried pastry triangles stuffed with spiced potatoes and peas, served hot with tamarind and mint chutney.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'aloo-tikki',
              name: 'Aloo Tikki',
              description:
                'Pan-fried potato and pea patties seasoned with chaat masala, served with yogurt and sweet-sour chutney.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'bhel-puri',
              name: 'Bhel Puri',
              description:
                'A crunchy Mumbai chaat of puffed rice, sev, onion, tomato, and chutneys tossed together moments before serving.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'kathi-roll',
              name: 'Kathi Roll',
              description:
                'A Kolkata street classic of flaky paratha wrapped around marinated chicken or paneer with onion and chutney.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'dahi-puri',
              name: 'Dahi Puri',
              description:
                'Crispy puri shells filled with potato, topped with cool yogurt, tamarind chutney, and a dusting of chaat masala.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'sev-puri',
              name: 'Sev Puri',
              description:
                'Flat puris topped with potato, chutneys, and a blizzard of crispy chickpea-flour sev for a bold chaat bite.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chaat',
              name: 'Papdi Chaat',
              description:
                'Crunchy fried papdi wafers piled with chickpeas, potato, yogurt, tamarind, and green chutney in every bite.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'indian-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'gulab-jamun',
              name: 'Gulab Jamun',
              description:
                'Soft milk-solid dumplings deep-fried until dark golden and soaked in rose and cardamom-scented sugar syrup.',
              prepTime: '20 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'rasgulla',
              name: 'Rasgulla',
              description:
                'Delicate spongy chenna cheese balls simmered in light sugar syrup, a Bengali classic light as a cloud.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'kheer',
              name: 'Kheer',
              description:
                'Creamy slow-cooked rice pudding simmered in whole milk with sugar, cardamom, saffron, and scattered rose petals.',
              prepTime: '10 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'jalebi',
              name: 'Jalebi',
              description:
                'Pretzel-shaped batter spirals deep-fried to a crisp, then dunked in saffron syrup for a sticky, sugary sweet.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'ladoo',
              name: 'Besan Ladoo',
              description:
                'Round gram flour sweets roasted in ghee with sugar and cardamom, rolled into golden balls for festive occasions.',
              prepTime: '10 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 20,
            },
            {
              slug: 'halwa',
              name: 'Gajar Halwa',
              description:
                'Slow-cooked carrot halwa stirred in ghee and milk until thick, sweetened with sugar and topped with cashews.',
              prepTime: '15 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'kulfi',
              name: 'Kulfi',
              description:
                'Dense Indian ice cream made from reduced milk and cream, set in a mold with pistachios and rose water.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'barfi',
              name: 'Barfi',
              description:
                'A fudge-like milk-solid sweet flavored with pistachio, coconut, or chocolate and cut into neat diamond shapes.',
              prepTime: '10 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 16,
            },
            {
              slug: 'malpua',
              name: 'Malpua',
              description:
                'Fluffy semolina pancakes fried in ghee and soaked in sugar syrup, garnished with chopped nuts and cream.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'ras-malai',
              name: 'Ras Malai',
              description:
                'Soft chenna cheese discs soaked in chilled saffron and cardamom-scented reduced milk for a luxurious dessert.',
              prepTime: '30 mins',
              cookTime: '40 mins',
              difficulty: 'Hard',
              servings: 8,
            },
          ],
        },
        {
          slug: 'indian-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'masala-dosa',
              name: 'Masala Dosa',
              description:
                'A crispy, wafer-thin fermented crepe from South India filled with spiced potato masala and served with chutneys.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'idli-sambar',
              name: 'Idli with Sambar',
              description:
                'Steamed fermented rice and lentil cakes served alongside a rich lentil and vegetable sambar and coconut chutney.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'poha',
              name: 'Poha',
              description:
                'Flattened rice flakes tempered with mustard seeds, curry leaves, onion, and turmeric for a light, quick breakfast.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'upma',
              name: 'Upma',
              description:
                'Roasted semolina porridge cooked with ghee, mustard seeds, vegetables, and curry leaves until fluffy and dry.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'aloo-paratha',
              name: 'Aloo Paratha',
              description:
                'Whole wheat flatbreads stuffed with spiced mashed potato, pan-fried in butter and served with yogurt and pickle.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'puri-bhaji',
              name: 'Puri Bhaji',
              description:
                'Deep-fried puffed wheat puris served alongside a dry potato subzi seasoned with mustard seeds and turmeric.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'medu-vada',
              name: 'Medu Vada',
              description:
                'Crispy ring-shaped urad lentil fritters served with coconut chutney and sambar, a South Indian breakfast staple.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'uttapam',
              name: 'Uttapam',
              description:
                'Thick, fluffy rice and lentil pancakes topped with onions, tomatoes, and green chilies cooked on a griddle.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'halwa-puri',
              name: 'Halwa Puri',
              description:
                'A festive Pakistani-influenced breakfast of semolina halwa, fluffy puris, spiced chickpeas, and pickled mango.',
              prepTime: '25 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'paratha',
              name: 'Paratha with Pickle',
              description:
                'Flaky layered whole wheat flatbreads cooked with ghee until golden, served with tangy mango pickle and lassi.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'indian-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'masala-chai',
              name: 'Masala Chai',
              description:
                'Robust black tea simmered with ginger, cardamom, cinnamon, and cloves, sweetened and finished with whole milk.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'mango-lassi',
              name: 'Mango Lassi',
              description:
                'A thick, refreshing blend of ripe Alphonso mango pulp and cool yogurt, lightly sweetened and scented with cardamom.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'lassi',
              name: 'Salted Lassi',
              description:
                'Chilled yogurt blended with water, a pinch of salt, roasted cumin, and fresh mint for a savory cooling drink.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'nimbu-pani',
              name: 'Nimbu Pani',
              description:
                'Indian fresh lemonade of squeezed lime, sugar or salt, cumin, and cold water—essential on a hot day.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'thandai',
              name: 'Thandai',
              description:
                'A festive chilled milk drink blended with almonds, poppy seeds, rose petals, and saffron for Holi celebrations.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'aam-panna',
              name: 'Aam Panna',
              description:
                'A cooling summer drink of raw green mango boiled and blended with cumin, mint, and sugar to beat the heat.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'rose-sharbat',
              name: 'Rose Sharbat',
              description:
                'A fragrant chilled drink of rose syrup, milk or water, and basil seeds, poured over ice for a floral refresher.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'filter-coffee',
              name: 'Filter Coffee',
              description:
                'South Indian decoction coffee brewed in a traditional filter, blended with hot milk and frothed between tumblers.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'jal-jeera',
              name: 'Jal Jeera',
              description:
                'A zesty spiced water of roasted cumin, tamarind, black salt, and mint, served iced as a digestive aperitif.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'coconut-water',
              name: 'Fresh Coconut Water',
              description:
                'Naturally sweet electrolyte-rich water drunk straight from a green tender coconut, cut open at street stalls.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
          ],
        },
      ],
    },
    {
      slug: 'mexican',
      name: 'Mexican Cuisine',
      country: 'Mexico',
      flag: '🇲🇽',
      description:
        'Mexican cuisine is a UNESCO-recognized culinary heritage built on ancient Mesoamerican foundations—corn, beans, and chilies—layered with Spanish colonial influences. From the complex, chocolate-laced moles of Oaxaca to the bright ceviches of the Pacific coast, it is a cuisine of extraordinary regional depth, bold contrasts, and festive generosity.',
      categories: [
        {
          slug: 'mexican-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'tacos-al-pastor',
              name: 'Tacos al Pastor',
              description:
                'Pork marinated in dried chilies and achiote, shaved from a vertical spit onto corn tortillas with pineapple and onion.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'mole-negro',
              name: 'Mole Negro',
              description:
                'An Oaxacan masterpiece of over thirty ingredients—dried chilies, chocolate, charred bread, and spices—over turkey.',
              prepTime: '60 mins',
              cookTime: '3 hrs',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'enchiladas',
              name: 'Enchiladas Verdes',
              description:
                'Corn tortillas rolled around chicken, bathed in a bright tomatillo salsa verde and crowned with crema and cheese.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'chiles-en-nogada',
              name: 'Chiles en Nogada',
              description:
                'Roasted poblano peppers stuffed with a sweet-savory picadillo, draped in walnut cream sauce and pomegranate seeds.',
              prepTime: '45 mins',
              cookTime: '30 mins',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'carnitas',
              name: 'Carnitas',
              description:
                'Pork shoulder slow-braised in lard until tender, then crisped and pulled for tacos or served with fresh tortillas.',
              prepTime: '15 mins',
              cookTime: '3 hrs',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'tamales',
              name: 'Tamales de Rajas con Queso',
              description:
                'Masa dough parcels stuffed with roasted poblano strips and melted cheese, steamed in corn husks.',
              prepTime: '60 mins',
              cookTime: '1 hr',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'fish-tacos',
              name: 'Fish Tacos Baja-Style',
              description:
                'Beer-battered white fish in warm corn tortillas topped with cabbage slaw, crema, and a squeeze of lime.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'quesadillas',
              name: 'Quesadillas with Huitlacoche',
              description:
                'Crispy griddle-pressed flour tortillas filled with corn truffle, Oaxacan cheese, and epazote herb.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'carne-asada',
              name: 'Carne Asada',
              description:
                'Skirt steak marinated in citrus, garlic, and spices, grilled over high heat and sliced thin for tacos or burritos.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'cochinita-pibil',
              name: 'Cochinita Pibil',
              description:
                'A Yucatecan tradition of pork marinated in achiote and bitter orange, wrapped in banana leaves and pit-roasted.',
              prepTime: '30 mins',
              cookTime: '4 hrs',
              difficulty: 'Hard',
              servings: 8,
            },
          ],
        },
        {
          slug: 'mexican-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'pozole',
              name: 'Pozole Rojo',
              description:
                'A festive Guerreran soup of pork and hominy in a deep red guajillo and ancho chili broth, topped with cabbage and radish.',
              prepTime: '30 mins',
              cookTime: '2 hrs',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'menudo',
              name: 'Menudo',
              description:
                'A bold, slow-simmered tripe stew in a dried red chili broth with hominy, oregano, and a splash of lime.',
              prepTime: '30 mins',
              cookTime: '4 hrs',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'sopa-de-lima',
              name: 'Sopa de Lima',
              description:
                'A Yucatecan chicken broth brightened with charred sweet limes and garnished with crispy tortilla strips.',
              prepTime: '15 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'caldo-de-res',
              name: 'Caldo de Res',
              description:
                'A hearty Mexican beef and vegetable broth with corn on the cob, chayote, and a squeeze of fresh lime.',
              prepTime: '20 mins',
              cookTime: '2 hrs',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'black-bean-soup',
              name: 'Black Bean Soup',
              description:
                'Silky blended black beans seasoned with epazote and chipotle, topped with queso fresco and crispy tortilla.',
              prepTime: '10 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'sopa-azteca',
              name: 'Sopa Azteca',
              description:
                'Tortilla soup of crispy fried strips in a smoky tomato-pasilla broth with avocado, sour cream, and cheese.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'caldo-tlalpeno',
              name: 'Caldo Tlalpeño',
              description:
                'A smoky Mexico City soup of chicken, chickpeas, and chipotle, finished with fresh avocado and lime.',
              prepTime: '15 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'birria',
              name: 'Birria de Res',
              description:
                'Jalisco-style beef slow-cooked in a dried chili adobo until falling apart, served in its rich consommé broth.',
              prepTime: '30 mins',
              cookTime: '3 hrs',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'sopa-de-fideo',
              name: 'Sopa de Fideo',
              description:
                'Toasted thin noodles simmered in a tomato and garlic broth, Mexico\'s quick and comforting weeknight soup.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'crema-de-elote',
              name: 'Crema de Elote',
              description:
                'A velvety sweet corn cream soup blended with butter, epazote, and a swirl of crema for a rich autumn bowl.',
              prepTime: '10 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'mexican-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'elote',
              name: 'Elote',
              description:
                'Grilled corn on the cob slathered in crema, rolled in cotija cheese, and dusted with chili powder and lime.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'torta',
              name: 'Torta Ahogada',
              description:
                'A birote bread roll stuffed with carnitas and drowned in a fiery tomato and arbol chili sauce from Guadalajara.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'tostadas',
              name: 'Tostadas de Ceviche',
              description:
                'Crispy fried corn tostadas topped with fresh lime-cured fish, avocado, red onion, and a drizzle of hot sauce.',
              prepTime: '20 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'flautas',
              name: 'Flautas',
              description:
                'Tightly rolled corn tortillas filled with chicken or potato, deep-fried until crispy and served with guacamole.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'churros',
              name: 'Churros con Chocolate',
              description:
                'Ridged fried choux pastry dusted in cinnamon sugar, served with a thick dark Mexican chocolate dipping sauce.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'gorditas',
              name: 'Gorditas',
              description:
                'Thick stuffed masa cakes split open and filled with beans, cheese, chicharrón, or salsa for a hearty street bite.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'tlayudas',
              name: 'Tlayuda',
              description:
                'A large, crispy Oaxacan tortilla smeared with black bean paste, Oaxacan cheese, and a choice of toppings.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'huaraches',
              name: 'Huaraches',
              description:
                'Sandal-shaped thick masa flatbreads topped with refried beans, salsa, avocado, and your choice of protein.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'tetelas',
              name: 'Tetelas',
              description:
                'Triangular Oaxacan masa pockets stuffed with black beans and cheese, toasted on a comal until golden.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'memelas',
              name: 'Memelas',
              description:
                'Oval masa cakes from Mexico City patted thin and topped with frijoles, salsa, and a crumbling of fresh cheese.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'mexican-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'tres-leches',
              name: 'Tres Leches Cake',
              description:
                'A light sponge soaked in three milks—evaporated, condensed, and heavy cream—topped with whipped cream and cinnamon.',
              prepTime: '25 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'flan',
              name: 'Flan',
              description:
                'A silky Mexican crème caramel set in a pool of golden caramel sauce, a timeless dessert of Spanish heritage.',
              prepTime: '15 mins',
              cookTime: '50 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'arroz-con-leche',
              name: 'Arroz con Leche',
              description:
                'Creamy Mexican rice pudding cooked with cinnamon stick and lemon peel in whole milk until thick and fragrant.',
              prepTime: '5 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'bunuelos',
              name: 'Buñuelos',
              description:
                'Thin, crispy fried dough discs dusted with sugar and cinnamon, a traditional Christmas and Day of the Dead sweet.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 10,
            },
            {
              slug: 'cajeta-crepes',
              name: 'Cajeta Crepes',
              description:
                'Delicate crepes folded around a generous pour of cajeta—a rich Mexican goat-milk caramel—and topped with pecans.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chongos-zamoranos',
              name: 'Chongos Zamoranos',
              description:
                'Tender curd knots simmered slowly in a cinnamon and piloncillo syrup until caramelized and silky in texture.',
              prepTime: '20 mins',
              cookTime: '3 hrs',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'gelatinas',
              name: 'Gelatinas de Mosaico',
              description:
                'Colorful mosaic gelatin dessert of layered fruit-flavored cubes suspended in a creamy milk jelly.',
              prepTime: '30 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'polvorones',
              name: 'Polvorones',
              description:
                'Crumbly Mexican shortbread cookies made with lard and pecans, dusted in powdered sugar and melt-in-the-mouth.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 24,
            },
            {
              slug: 'mazapan',
              name: 'Mazapán',
              description:
                'A crumbly, melt-in-the-mouth Mexican peanut candy pressed into rounds, iconic at every corner store.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 12,
            },
            {
              slug: 'churros-rellenos',
              name: 'Churros Rellenos',
              description:
                'Hollow fried churros piped full of cajeta, Nutella, or pastry cream for an indulgent Mexican street dessert.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 6,
            },
          ],
        },
        {
          slug: 'mexican-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'chilaquiles',
              name: 'Chilaquiles',
              description:
                'Fried tortilla chips briefly simmered in salsa verde or roja, topped with fried eggs, crema, and queso fresco.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'huevos-rancheros',
              name: 'Huevos Rancheros',
              description:
                'Fried eggs served on warm corn tortillas blanketed with a chunky tomato-chili ranch sauce.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'molletes',
              name: 'Molletes',
              description:
                'Split bolillo rolls spread with refried beans and melted cheese, toasted open-face and served with fresh pico de gallo.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'pan-dulce',
              name: 'Pan Dulce',
              description:
                'An assortment of Mexican sweet breads—conchas, cuernos, and polvorones—best enjoyed with a warm café de olla.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 12,
            },
            {
              slug: 'enfrijoladas',
              name: 'Enfrijoladas',
              description:
                'Corn tortillas dipped in a smooth black bean sauce and folded around chicken or cheese, topped with crema.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'machaca-con-huevo',
              name: 'Machaca con Huevo',
              description:
                'Shredded dried beef scrambled with eggs, tomato, chili, and onion, a hearty Norteño ranch breakfast.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'hot-cakes',
              name: 'Hot Cakes Mexicanos',
              description:
                'Fluffy Mexican-style pancakes served with cajeta syrup, fresh fruit, and a sprinkle of cinnamon sugar.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tostadas-con-frijoles',
              name: 'Tostadas con Frijoles',
              description:
                'Crispy corn tostadas piled with warm refried beans, sliced avocado, and a drizzle of salsa for a quick breakfast.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'tamales-de-elote',
              name: 'Tamales de Elote',
              description:
                'Sweet fresh corn tamales steamed in their own husks with a moist, tender masa that needs no filling.',
              prepTime: '30 mins',
              cookTime: '1 hr',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'cafe-de-olla',
              name: 'Café de Olla',
              description:
                'Rustic Mexican coffee brewed in an earthenware pot with piloncillo, cinnamon, and a touch of clove.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'mexican-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'horchata',
              name: 'Horchata',
              description:
                'A chilled rice milk drink blended with cinnamon and vanilla, sweet, creamy, and infinitely refreshing.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'agua-de-jamaica',
              name: 'Agua de Jamaica',
              description:
                'Tart, jewel-red hibiscus flower tea sweetened with sugar and served over ice, a staple at every Mexican market.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'tamarindo-agua',
              name: 'Agua de Tamarindo',
              description:
                'Sweet and sour tamarind pulp dissolved in water, sweetened to taste and served over ice with a chili rim.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'margarita',
              name: 'Margarita',
              description:
                'The iconic Mexican cocktail of blanco tequila, triple sec, and fresh lime shaken and served in a salt-rimmed glass.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'michelada',
              name: 'Michelada',
              description:
                'A savory beer cocktail of Mexican lager, lime juice, hot sauce, and Worcestershire served over ice in a chili-salt rim.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'mezcal',
              name: 'Mezcal',
              description:
                'A smoky artisanal spirit distilled from roasted agave hearts in Oaxaca, sipped neat with an orange and sal de gusano.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'tepache',
              name: 'Tepache',
              description:
                'A mildly fermented pineapple peel drink brewed with piloncillo and cinnamon for a tangy, lightly fizzy refresher.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'champurrado',
              name: 'Champurrado',
              description:
                'A thick, warming Mexican hot chocolate thickened with masa harina and spiced with cinnamon and piloncillo.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'atole',
              name: 'Atole',
              description:
                'A comforting hot drink of masa, milk, and piloncillo flavored with vanilla or strawberry, served alongside tamales.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tequila-sunrise',
              name: 'Tequila Sunrise',
              description:
                'Tequila and fresh orange juice layered with grenadine for a sunrise gradient, Mexico\'s most photogenic cocktail.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
          ],
        },
      ],
    },
    {
      slug: 'turkish',
      name: 'Turkish Cuisine',
      country: 'Turkey',
      flag: '🇹🇷',
      description:
        'Turkish cuisine is the grand heir of Ottoman palace cooking, blending Central Asian, Middle Eastern, and Mediterranean traditions into a rich, generous table. From the wood-fire grills of Adana to the seafood-laden mezes of Istanbul\'s Bosphorus shores, it is a cuisine of bold flavors, convivial hospitality, and centuries of refined craft.',
      categories: [
        {
          slug: 'turkish-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'iskender-kebab',
              name: 'İskender Kebab',
              description:
                'Thinly sliced döner lamb over torn pide bread, drenched in hot tomato sauce and poured-over browned butter.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'adana-kebab',
              name: 'Adana Kebab',
              description:
                'Hand-minced spicy lamb kofta pressed onto wide flat skewers and grilled over charcoal, served with bulgur pilav.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'doner-kebab',
              name: 'Döner Kebab',
              description:
                'Seasoned lamb and veal stacked on a vertical spit, slowly roasted and shaved to order into flatbread with salad.',
              prepTime: '30 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'lahmacun',
              name: 'Lahmacun',
              description:
                'Paper-thin flatbread topped with a spiced minced meat paste, baked crisp and rolled with parsley and lemon.',
              prepTime: '25 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'imam-bayildi',
              name: 'İmam Bayıldı',
              description:
                'Whole aubergine slow-braised in olive oil and stuffed with a fragrant mixture of onion, tomato, and garlic.',
              prepTime: '20 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'manti',
              name: 'Mantı',
              description:
                'Tiny hand-pinched lamb dumplings boiled and served with garlicky yogurt, browned butter, and dried mint.',
              prepTime: '60 mins',
              cookTime: '15 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'hunkar-begendi',
              name: 'Hünkâr Beğendi',
              description:
                'Ottoman braised lamb served over a velvety smoked aubergine and béchamel purée—Sultan\'s Delight.',
              prepTime: '20 mins',
              cookTime: '1 hr',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'pide',
              name: 'Pide',
              description:
                'Boat-shaped Turkish flatbread baked with toppings of minced meat, egg, or kaşar cheese and fresh herbs.',
              prepTime: '30 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'kofte',
              name: 'Köfte',
              description:
                'Seasoned ground lamb and beef patties mixed with onion and spices, grilled and served with flatbread and salad.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'shish-kebab',
              name: 'Şiş Kebab',
              description:
                'Cubes of marinated lamb or chicken threaded onto skewers with peppers and onions, grilled over charcoal.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'turkish-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'mercimek-corbasi',
              name: 'Mercimek Çorbası',
              description:
                'Velvety red lentil soup seasoned with cumin and paprika, finished with a drizzle of butter and dried mint.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'yayla-corbasi',
              name: 'Yayla Çorbası',
              description:
                'A comforting highland yogurt and rice soup tempered with eggs and finished with a mint-butter drizzle.',
              prepTime: '10 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'ezogelin-corbasi',
              name: 'Ezogelin Çorbası',
              description:
                'A warming red lentil and bulgur soup with tomato paste, dried mint, and paprika, named after a bride of legend.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tarhana-corbasi',
              name: 'Tarhana Çorbası',
              description:
                'A thick, tangy soup made from fermented dried tarhana powder dissolved in broth—Turkey\'s oldest instant soup.',
              prepTime: '5 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'domates-corbasi',
              name: 'Domates Çorbası',
              description:
                'A smooth Turkish tomato soup enriched with butter and finished with cream, simple and deeply satisfying.',
              prepTime: '10 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'dugun-corbasi',
              name: 'Düğün Çorbası',
              description:
                'A rich lamb and yogurt wedding soup finished with a paprika-butter tarka, served at Turkish celebrations.',
              prepTime: '15 mins',
              cookTime: '1 hr',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'tavuk-corbasi',
              name: 'Tavuk Çorbası',
              description:
                'A nourishing chicken and vermicelli soup made from a golden homemade broth with vegetables and lemon.',
              prepTime: '15 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'iskembe-corbasi',
              name: 'İşkembe Çorbası',
              description:
                'A restorative tripe soup thickened with egg yolk and vinegar, traditionally eaten late at night in Istanbul.',
              prepTime: '20 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'patlican-corbasi',
              name: 'Patlıcan Çorbası',
              description:
                'Smoky roasted aubergine blended into a velvety soup with yogurt, garlic, and a drizzle of olive oil.',
              prepTime: '15 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'sebze-corbasi',
              name: 'Sebze Çorbası',
              description:
                'A rustic medley of seasonal vegetables simmered in broth with a butter and tomato paste base.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'turkish-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'simit',
              name: 'Simit',
              description:
                'A circular sesame-coated bread ring with a crispy crust and chewy interior, sold from carts across every Turkish city.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'borek',
              name: 'Börek',
              description:
                'Layers of thin yufka pastry filled with feta cheese and parsley or spiced minced meat and baked until golden.',
              prepTime: '30 mins',
              cookTime: '35 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'midye-dolma',
              name: 'Midye Dolma',
              description:
                'Mussels stuffed with spiced herbed rice and lemon-brightened, sold by the tray along Istanbul\'s waterfront.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'balik-ekmek',
              name: 'Balık Ekmek',
              description:
                'Grilled mackerel fillets tucked into a crusty white roll with onion, lettuce, and a squeeze of lemon.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'gozleme',
              name: 'Gözleme',
              description:
                'Hand-rolled yufka flatbreads filled with cheese and spinach or minced meat, folded and cooked on a griddle.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'kumpir',
              name: 'Kumpir',
              description:
                'A giant baked potato split and mashed with butter and cheese, then piled with endless toppings of your choice.',
              prepTime: '10 mins',
              cookTime: '1 hr',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'durum',
              name: 'Dürüm',
              description:
                'Thin lavash bread tightly rolled around grilled meat, salad, and spicy ezme sauce for a portable Turkish wrap.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'tantuni',
              name: 'Tantuni',
              description:
                'Mersin-style sautéed beef with tomato, peppers, and cumin rolled into a thin tortilla-like bread.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'kokurec',
              name: 'Kokoreç',
              description:
                'Seasoned lamb intestines wound around a skewer and roasted over charcoal, chopped and served in crusty bread.',
              prepTime: '30 mins',
              cookTime: '30 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'karadeniz-pide',
              name: 'Karadeniz Pidesi',
              description:
                'Black Sea pide filled with minced beef, butter, and a cracked egg baked until the dough puffs and blisters.',
              prepTime: '25 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'turkish-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'baklava',
              name: 'Baklava',
              description:
                'Dozens of butter-brushed filo layers packed with pistachios or walnuts and soaked in a fragrant sugar syrup.',
              prepTime: '45 mins',
              cookTime: '45 mins',
              difficulty: 'Hard',
              servings: 24,
            },
            {
              slug: 'kunefe',
              name: 'Künefe',
              description:
                'Shredded kadayif pastry sandwiching molten unsalted cheese, baked in a copper pan and drenched in syrup.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'turkish-delight',
              name: 'Turkish Delight',
              description:
                'Soft, jewel-like squares of gel-set confection flavored with rose water or mastic, dusted in powdered sugar.',
              prepTime: '15 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 30,
            },
            {
              slug: 'sutlac',
              name: 'Sütlaç',
              description:
                'A baked rice pudding caramelized under the grill until the surface is burnished golden, served chilled.',
              prepTime: '10 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'kazandibi',
              name: 'Kazandibi',
              description:
                'A deliberately scorched milk pudding with a caramelized underside, rolled and dusted with cinnamon.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'asure',
              name: 'Aşure',
              description:
                'A thick biblical pudding of boiled grains, legumes, dried fruits, and nuts shared generously on Muharram.',
              prepTime: '20 mins',
              cookTime: '1 hr',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'helva',
              name: 'Helva',
              description:
                'A dense, crumbly semolina or tahini sweet cooked in butter with pine nuts and raisins, served warm.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'muhallebi',
              name: 'Muhallebi',
              description:
                'A silky chilled milk pudding set with rice flour and perfumed with rose water, garnished with crushed pistachios.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'revani',
              name: 'Revani',
              description:
                'A semolina and flour sponge soaked in lemon sugar syrup until glistening, topped with desiccated coconut.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 12,
            },
            {
              slug: 'sekerpare',
              name: 'Şekerpare',
              description:
                'Soft semolina and almond cookies baked then bathed in lemon sugar syrup with a whole almond on each.',
              prepTime: '20 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 20,
            },
          ],
        },
        {
          slug: 'turkish-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'menemen',
              name: 'Menemen',
              description:
                'Eggs scrambled into a sizzling pan of tomatoes, green peppers, and olive oil for Turkey\'s ultimate brunch.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'sucuklu-yumurta',
              name: 'Sucuklu Yumurta',
              description:
                'Spiced Turkish sausage slices pan-fried until the fat renders, then eggs cracked in to cook in the spiced fat.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'kaymak-with-honey',
              name: 'Kaymak with Honey',
              description:
                'Thick clotted cream from water buffalo milk drizzled with amber raw honey and eaten with warm simit.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'tahini-pekmez',
              name: 'Tahini Pekmez',
              description:
                'Swirled sesame paste and grape molasses on a plate, dipped into with torn bread for a classic Anatolian breakfast.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'turkish-cheese-platter',
              name: 'Turkish Cheese Platter',
              description:
                'A spread of white beyaz peynir, aged kaşar, and tulum cheeses served with olives, tomatoes, and fresh bread.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'pastirma-eggs',
              name: 'Pastırmalı Yumurta',
              description:
                'Spiced air-dried beef pastırma slices fried and served with runny eggs, a rich and aromatic Turkish breakfast.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'cheese-borek-breakfast',
              name: 'Peynirli Börek',
              description:
                'Flaky yufka pastry stuffed with feta and parsley, pan-fried in butter until golden and eaten warm at the morning table.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'simit-with-cheese',
              name: 'Simit with White Cheese',
              description:
                'A sesame-encrusted bread ring broken and eaten alongside slices of creamy beyaz peynir and sliced tomatoes.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'beyaz-peynir-olives',
              name: 'Beyaz Peynir with Olives',
              description:
                'Brined white cheese paired with cured black and green olives—the bedrock of any Turkish kahvaltı spread.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tomato-cucumber-breakfast',
              name: 'Tomato and Cucumber Salad',
              description:
                'Ripe vine tomatoes and cool cucumber sliced and drizzled with olive oil and fresh dill for a classic side.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'turkish-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'turkish-tea',
              name: 'Çay',
              description:
                'Strong black tea brewed in a two-tiered çaydanlık and poured into tulip glasses, the heartbeat of Turkish hospitality.',
              prepTime: '5 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'turkish-coffee',
              name: 'Turkish Coffee',
              description:
                'Finely ground coffee simmered with water and sugar in a cezve until frothy, served with the grounds in the cup.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Medium',
              servings: 1,
            },
            {
              slug: 'ayran',
              name: 'Ayran',
              description:
                'A savory chilled drink of yogurt whisked with water and a pinch of salt until frothy, the perfect kebab companion.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'salgam',
              name: 'Şalgam',
              description:
                'A tart, earthy fermented turnip and purple carrot juice from Adana, drunk alongside Adana kebab.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'sahlep',
              name: 'Sahlep',
              description:
                'A warming winter drink of orchid-root powder thickened into a creamy hot beverage dusted with cinnamon.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'boza',
              name: 'Boza',
              description:
                'A thick, mildly fermented wheat drink with a sweet-sour taste, served with roasted chickpeas in winter streets.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'raki',
              name: 'Rakı',
              description:
                'Turkey\'s national anise spirit mixed with cold water until it turns milky white, sipped alongside meze at dinner.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'limonata',
              name: 'Limonata',
              description:
                'Fresh Turkish lemonade blended from whole lemons for a cloud-white, intensely citrusy fizzy drink.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'menenggic-coffee',
              name: 'Menengiç Coffee',
              description:
                'A caffeine-free coffee substitute brewed from wild terebinth berries, nutty and aromatic from southeast Turkey.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'visne-suyu',
              name: 'Vişne Suyu',
              description:
                'Chilled sour cherry juice pressed from morello cherries, sweet-tart and brilliantly red, a beloved Turkish classic.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
      ],
    },
    {
      slug: 'french',
      name: 'French Cuisine',
      country: 'France',
      flag: '🇫🇷',
      description:
        'French cuisine is the cornerstone of Western culinary tradition, codified by generations of chefs who elevated cooking to a fine art. From the rustic braises of Burgundy and the bouillabaisse of Marseille to the meticulous pastry craft of Parisian pâtisseries, it is a cuisine built on technique, terroir, and an uncompromising respect for quality ingredients.',
      categories: [
        {
          slug: 'french-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'boeuf-bourguignon',
              name: 'Bœuf Bourguignon',
              description:
                'Beef braised low and slow in Burgundy red wine with lardons, pearl onions, and mushrooms until deeply tender.',
              prepTime: '30 mins',
              cookTime: '3 hrs',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'coq-au-vin',
              name: 'Coq au Vin',
              description:
                'Chicken slowly braised in red wine with mushrooms, lardons, and thyme until the sauce is glossy and intense.',
              prepTime: '25 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'duck-confit',
              name: 'Duck Confit',
              description:
                'Duck legs cured overnight in salt and herbs then slow-cooked submerged in their own fat until silky and rich.',
              prepTime: '20 mins',
              cookTime: '2 hrs 30 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'sole-meuniere',
              name: 'Sole Meunière',
              description:
                'Dover sole dusted in flour and pan-fried in clarified butter, finished with lemon juice and fresh parsley.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'cassoulet',
              name: 'Cassoulet',
              description:
                'A slow-baked Languedoc casserole of white beans, duck confit, Toulouse sausages, and pork under a golden crust.',
              prepTime: '30 mins',
              cookTime: '3 hrs',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'blanquette-de-veau',
              name: 'Blanquette de Veau',
              description:
                'Veal and vegetables poached in a pale broth enriched with cream and egg yolks for a delicate white stew.',
              prepTime: '25 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'steak-frites',
              name: 'Steak Frites',
              description:
                'Entrecôte steak cooked to a perfect medium-rare in sizzling butter with thyme, served alongside crispy twice-fried chips.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'quiche-lorraine',
              name: 'Quiche Lorraine',
              description:
                'A buttery shortcrust tart filled with a custard of eggs, cream, and smoked lardons, baked until just set.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'ratatouille',
              name: 'Ratatouille',
              description:
                'A Provençal vegetable braise of aubergine, courgette, tomato, and peppers in olive oil and fresh herbs.',
              prepTime: '20 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'bouillabaisse',
              name: 'Bouillabaisse',
              description:
                'A Marseillaise seafood stew of multiple fish and shellfish in saffron-scented broth, served with rouille and toasted bread.',
              prepTime: '30 mins',
              cookTime: '45 mins',
              difficulty: 'Hard',
              servings: 6,
            },
          ],
        },
        {
          slug: 'french-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'french-onion-soup',
              name: 'French Onion Soup',
              description:
                'Caramelized onions slow-cooked in beef broth, ladled into crocks and blanketed under gratinéed Gruyère.',
              prepTime: '15 mins',
              cookTime: '1 hr 15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'vichyssoise',
              name: 'Vichyssoise',
              description:
                'A silky chilled leek and potato soup enriched with cream and seasoned delicately, topped with fresh chives.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'pot-au-feu',
              name: 'Pot-au-Feu',
              description:
                'France\'s emblematic one-pot of beef cuts and root vegetables simmered in broth, served in two distinct courses.',
              prepTime: '20 mins',
              cookTime: '3 hrs',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'soupe-au-pistou',
              name: 'Soupe au Pistou',
              description:
                'A Provençal vegetable and bean soup finished tableside with a vibrant basil, garlic, and olive oil pistou.',
              prepTime: '25 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'bisque-de-homard',
              name: 'Bisque de Homard',
              description:
                'A luxurious, velvety lobster bisque made from roasted shells, cream, cognac, and a rich shellfish stock.',
              prepTime: '30 mins',
              cookTime: '50 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'garbure',
              name: 'Garbure',
              description:
                'A Gascon peasant soup so thick with cabbage, beans, and duck confit it can stand a spoon upright.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'veloute-de-champignons',
              name: 'Velouté de Champignons',
              description:
                'A silky wild mushroom cream soup made from a classic velouté base, garnished with truffle oil and chervil.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'potage-saint-germain',
              name: 'Potage Saint-Germain',
              description:
                'A vibrant green split pea and leek soup finished with butter and served with croûtons and crème fraîche.',
              prepTime: '10 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'soupe-de-poisson',
              name: 'Soupe de Poisson',
              description:
                'A robust blended Provençal fish soup strained smooth, served with rouille, grated Gruyère, and grilled croutons.',
              prepTime: '20 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'soupe-a-la-tomate',
              name: 'Soupe à la Tomate',
              description:
                'Slow-roasted French tomatoes blended with shallots, tarragon, and a touch of cream into a velvety red soup.',
              prepTime: '10 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'french-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'crepe',
              name: 'Crêpe',
              description:
                'A paper-thin Breton buckwheat or sweet flour crêpe cooked on a billig griddle and filled to order on the street.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'jambon-beurre',
              name: 'Jambon-Beurre',
              description:
                'A French baguette split and generously buttered, layered with thin slices of quality cooked ham.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'frites',
              name: 'Frites',
              description:
                'Double-fried Belgian-style thin chips sold in paper cones from street stands, served with mayonnaise.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'socca',
              name: 'Socca',
              description:
                'A crispy Niçoise chickpea-flour pancake blistered under a wood fire, broken into rough pieces and seasoned with pepper.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'merguez-frites',
              name: 'Merguez Frites',
              description:
                'Spicy North African lamb sausages grilled and served in a baguette with frites and harissa—a Paris street staple.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'galette-bretonne',
              name: 'Galette Bretonne',
              description:
                'A buckwheat crêpe from Brittany filled with ham, Emmental, and a runny egg cooked directly on the galette.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'tarte-flambee',
              name: 'Tarte Flambée',
              description:
                'An Alsatian thin-crust flatbread spread with crème fraîche, smoked lardons, and onions, baked at furnace heat.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'pain-au-chocolat',
              name: 'Pain au Chocolat',
              description:
                'A rectangular flaky puff pastry encasing two dark chocolate batons, buttery and warm from the boulangerie oven.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'gaufre-liege',
              name: 'Gaufre de Liège',
              description:
                'A caramelized yeasted Belgian-style waffle studded with pearl sugar, eaten warm from a street iron.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'croissant-street',
              name: 'Croissant',
              description:
                'The icon of French viennoiserie—hundreds of laminated butter layers rolled into a crescent and baked to a shatter.',
              prepTime: '60 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 8,
            },
          ],
        },
        {
          slug: 'french-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'creme-brulee',
              name: 'Crème Brûlée',
              description:
                'A silky vanilla baked custard chilled until set, then torched with sugar into a crackling caramel crust.',
              prepTime: '15 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'tarte-tatin',
              name: 'Tarte Tatin',
              description:
                'An upside-down caramelized apple tart baked under puff pastry and dramatically flipped onto the serving plate.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'mousse-au-chocolat',
              name: 'Mousse au Chocolat',
              description:
                'Airy dark chocolate mousse made from melted Valrhona chocolate folded with whipped egg whites and cream.',
              prepTime: '20 mins',
              cookTime: '5 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'mille-feuille',
              name: 'Mille-Feuille',
              description:
                'Crispy puff pastry layers alternating with vanilla pastry cream and topped with fondant icing piped in a feather pattern.',
              prepTime: '45 mins',
              cookTime: '25 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'eclair',
              name: 'Éclair',
              description:
                'A choux pastry log piped with vanilla or coffee cream and glazed with a smooth chocolate fondant on top.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Hard',
              servings: 10,
            },
            {
              slug: 'ile-flottante',
              name: 'Île Flottante',
              description:
                'Pillowy poached meringue islands floating on crème anglaise, scattered with toasted almonds and caramel threads.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'clafoutis',
              name: 'Clafoutis',
              description:
                'A Limousin batter pudding of cherries baked in a thick eggy custard until puffed golden like a sweet Dutch baby.',
              prepTime: '10 mins',
              cookTime: '35 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'paris-brest',
              name: 'Paris-Brest',
              description:
                'A ring of choux pastry split and filled with hazelnut praline mousseline cream, dusted with icing sugar.',
              prepTime: '40 mins',
              cookTime: '35 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'madeleines',
              name: 'Madeleines',
              description:
                'Shell-shaped financier cakes with a characteristic hump, baked golden and flavored with lemon zest and browned butter.',
              prepTime: '15 mins',
              cookTime: '12 mins',
              difficulty: 'Easy',
              servings: 12,
            },
            {
              slug: 'profiteroles',
              name: 'Profiteroles',
              description:
                'Choux puffs filled with vanilla ice cream and blanketed in a warm dark chocolate sauce at the table.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 6,
            },
          ],
        },
        {
          slug: 'french-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'tartine',
              name: 'Tartine',
              description:
                'A thick slice of sourdough or baguette toasted and spread with salted butter and fruit confiture.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'pain-perdu',
              name: 'Pain Perdu',
              description:
                'Thick slices of brioche soaked in sweetened vanilla egg custard and pan-fried in butter until deeply golden.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'omelette-fines-herbes',
              name: 'Omelette aux Fines Herbes',
              description:
                'A softly set, barely rolled French omelette flecked with chervil, tarragon, chives, and flat-leaf parsley.',
              prepTime: '5 mins',
              cookTime: '3 mins',
              difficulty: 'Medium',
              servings: 1,
            },
            {
              slug: 'oeufs-a-la-coque',
              name: 'Œufs à la Coque',
              description:
                'Soft-boiled eggs in their shell served in an egg cup with toasted mouillettes for dipping into the runny yolk.',
              prepTime: '2 mins',
              cookTime: '4 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'brioche-toastee',
              name: 'Brioche Toastée',
              description:
                'Thick slices of enriched brioche toasted and served with cultured butter and a selection of fruit jams.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'crepes-petit-dejeuner',
              name: 'Crêpes au Sucre',
              description:
                'Light morning crêpes sprinkled with caster sugar and a squeeze of fresh lemon, folded into quarters to eat.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'yaourt-fruits',
              name: 'Yaourt avec Fruits',
              description:
                'Creamy French whole-milk yogurt topped with seasonal fruits and a drizzle of lavender or acacia honey.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'viennoiseries',
              name: 'Viennoiseries',
              description:
                'A morning basket of fresh croissants, pains au chocolat, and pains aux raisins from the local boulangerie.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'cereales-au-lait',
              name: 'Céréales au Lait',
              description:
                'French muesli or granola soaked in cold whole milk with fresh strawberries and a dusting of vanilla sugar.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'oeuf-cocotte',
              name: 'Œuf Cocotte',
              description:
                'An egg baked in a ramekin with a spoon of cream and herbs until the white just sets around a still-runny yolk.',
              prepTime: '5 mins',
              cookTime: '12 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
        {
          slug: 'french-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'cafe-au-lait',
              name: 'Café au Lait',
              description:
                'Strong drip coffee poured simultaneously with hot steamed milk into a wide bowl, a classic French morning ritual.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'champagne',
              name: 'Champagne',
              description:
                'Prestige sparkling wine from the Champagne region, made by méthode champenoise with Chardonnay and Pinot grapes.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'kir-royale',
              name: 'Kir Royale',
              description:
                'A splash of black-currant crème de cassis topped with Champagne for a jewel-red, effervescent aperitif.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'pastis',
              name: 'Pastis',
              description:
                'Provençal anise-and-liquorice spirit diluted with ice-cold water until it turns a cloudy golden, the spirit of Marseille.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'cidre-breton',
              name: 'Cidre Breton',
              description:
                'Breton farmhouse cider pressed from bitter-sweet apples, gently sparkling and dry, served in a ceramic bolée.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'bordeaux-rouge',
              name: 'Bordeaux Rouge',
              description:
                'A classic Cabernet Sauvignon and Merlot blend from Bordeaux, structured and age-worthy with notes of blackcurrant.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'diabolo-menthe',
              name: 'Diabolo Menthe',
              description:
                'A refreshing French café drink of bright green mint syrup topped with sparkling lemonade, beloved by schoolchildren.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'orangina',
              name: 'Orangina',
              description:
                'The iconic French sparkling orange drink with real pulp, traditionally shaken before serving to wake the citrus sediment.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'grand-marnier',
              name: 'Grand Marnier',
              description:
                'A French liqueur of cognac and distilled bitter orange peel, sipped neat as a digestif or flambéed over crêpes.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'chocolat-chaud',
              name: 'Chocolat Chaud',
              description:
                'Rich Parisian-style hot chocolate made with melted dark chocolate and whole milk, thick, intense, and served in a pot.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
      ],
    },
    {
      slug: 'thai',
      name: 'Thai Cuisine',
      country: 'Thailand',
      flag: '🇹🇭',
      description:
        'Thai cuisine is a masterclass in the balance of sweet, sour, salty, spicy, and bitter in every dish. Anchored by fresh herbs, aromatic pastes, and coconut milk, it ranges from the fire-kissed street stalls of Bangkok to the delicate royal court traditions of Chiang Mai, united by a philosophy that every meal should simultaneously excite all the senses.',
      categories: [
        {
          slug: 'thai-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'pad-thai',
              name: 'Pad Thai',
              description:
                'Stir-fried rice noodles with shrimp or tofu, egg, bean sprouts, and peanuts in a tangy tamarind and fish sauce glaze.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'green-curry',
              name: 'Green Curry',
              description:
                'Chicken or vegetables simmered in fresh green chili coconut curry paste with Thai basil, kaffir lime, and eggplant.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'massaman-curry',
              name: 'Massaman Curry',
              description:
                'A mild, Persian-influenced coconut curry of slow-braised beef, potato, and peanuts with a warm spice profile.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'phat-kaphrao',
              name: 'Phat Kaphrao',
              description:
                'Thailand\'s beloved basil stir-fry of minced pork or chicken with bird\'s eye chilies, holy basil, and oyster sauce.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'pad-see-ew',
              name: 'Pad See Ew',
              description:
                'Wide rice noodles wok-charred with egg, Chinese broccoli, and tender beef in a sweet dark soy caramel.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'panang-curry',
              name: 'Panang Curry',
              description:
                'A thick, rich coconut curry with kaffir lime leaves and roasted peanuts ground into the paste for nutty depth.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'larb',
              name: 'Larb',
              description:
                'A Lao-influenced minced meat salad of pork or chicken seasoned with toasted rice powder, lime, fish sauce, and mint.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'som-tum',
              name: 'Som Tum',
              description:
                'A fiery green papaya salad pounded in a mortar with dried shrimp, tomatoes, long beans, and a lime-fish sauce dressing.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'red-curry',
              name: 'Red Curry',
              description:
                'Duck or pork in a bold red chili coconut curry enriched with bamboo shoots, cherry tomatoes, and Thai basil.',
              prepTime: '15 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'khao-pad',
              name: 'Khao Pad',
              description:
                'Thai fried rice stir-fried in a scorching wok with egg, jasmine rice, spring onion, and fish sauce.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
        {
          slug: 'thai-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'tom-yum-goong',
              name: 'Tom Yum Goong',
              description:
                'A bracing hot-and-sour prawn soup of lemongrass, galangal, kaffir lime, and bird\'s eye chilies in clear broth.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tom-kha-gai',
              name: 'Tom Kha Gai',
              description:
                'Chicken simmered in a coconut milk broth perfumed with galangal, lemongrass, and lime for a mellow, creamy soup.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'khao-soi',
              name: 'Khao Soi',
              description:
                'A northern Thai coconut curry noodle soup with braised chicken, fresh egg noodles, and crispy fried noodle topping.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'tom-saap',
              name: 'Tom Saap',
              description:
                'A deeply fragrant spicy pork rib soup with lemongrass, galangal, and roasted dried chilies from Isaan.',
              prepTime: '15 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'boat-noodles',
              name: 'Kuai Tiao Ruea',
              description:
                'Dark, intensely flavored boat noodles in a pork blood–enriched broth with pork slices and crispy garlic.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'tom-jued',
              name: 'Tom Jued',
              description:
                'A clean, mild clear soup of minced pork, glass noodles, and tofu in a light broth seasoned with white pepper.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'gaeng-om',
              name: 'Gaeng Om',
              description:
                'A northern Thai herbal curry stew with minimal coconut milk, packed with fresh dill, lemongrass, and pork ribs.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'kaeng-lueang',
              name: 'Kaeng Lueang',
              description:
                'A sour southern Thai yellow curry soup with fish, bamboo shoots, and turmeric-heavy paste and no coconut milk.',
              prepTime: '15 mins',
              cookTime: '25 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'nam-ngiao',
              name: 'Nam Ngiao',
              description:
                'A rich northern Thai noodle soup with pork ribs, fermented soybean, and dried tomato in a spiced broth.',
              prepTime: '20 mins',
              cookTime: '1 hr',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'khao-tom',
              name: 'Khao Tom',
              description:
                'Thai rice congee simmered until silky in a light chicken broth, topped with ginger, spring onion, and fried garlic.',
              prepTime: '5 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'thai-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'mango-sticky-rice',
              name: 'Mango Sticky Rice',
              description:
                'Sweet glutinous rice soaked in salted coconut cream served with slices of ripe yellow Nahm Dok Mai mango.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'gai-yang',
              name: 'Gai Yang',
              description:
                'Lemongrass-marinated whole chicken charcoal-grilled until the skin crisps and blackens at the edges.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'satay-thai',
              name: 'Satay',
              description:
                'Skewers of turmeric-marinated pork or chicken grilled over charcoal and served with peanut sauce and ajad.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'spring-rolls-thai',
              name: 'Poh Pia Thot',
              description:
                'Crispy deep-fried Thai spring rolls stuffed with glass noodles, carrots, and pork, dipped in sweet chili sauce.',
              prepTime: '25 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'thai-roti',
              name: 'Roti',
              description:
                'A flaky fried roti stretched paper-thin on a hot griddle, folded with egg and banana and drizzled with condensed milk.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'fish-cakes-thai',
              name: 'Tod Mun Pla',
              description:
                'Bouncy Thai fish cakes kneaded with red curry paste and kaffir lime, fried until golden and served with cucumber relish.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'isaan-sausage',
              name: 'Sai Krok Isaan',
              description:
                'Fermented sour pork sausages grilled until the skin pops, eaten with fresh ginger, cabbage, and bird\'s eye chilies.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'khanom-krok',
              name: 'Khanom Krok',
              description:
                'Coconut milk rice pudding baked in a cast-iron pan with a crispy outer shell and a molten sweet coconut center.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'thai-crepe',
              name: 'Kanom Bueng',
              description:
                'Crispy Thai crepes spread with sweet coconut or savory foythong cream and folded into half-moons to eat.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'grilled-pork-skewers',
              name: 'Moo Ping',
              description:
                'Juicy pork shoulder skewers marinated in coconut milk, oyster sauce, and palm sugar, grilled over charcoal.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
        {
          slug: 'thai-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'tub-tim-krob',
              name: 'Tub Tim Krob',
              description:
                'Red-dyed water chestnut rubies in a bowl of iced coconut cream—a jewel-like Thai dessert of textures.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'kluay-buat-chi',
              name: 'Kluay Buat Chi',
              description:
                'Plump banana pieces gently poached in sweet, lightly salted coconut milk—simple, creamy, and comforting.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'sangkaya',
              name: 'Sangkaya',
              description:
                'Thai coconut custard steamed inside a small pumpkin until silky, infused with pandan leaves and palm sugar.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'foi-thong',
              name: 'Foi Thong',
              description:
                'Golden egg yolk threads drawn through sugar syrup into thin strands, a delicate Thai-Portuguese confection.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'bua-loi',
              name: 'Bua Loi',
              description:
                'Tiny glutinous rice flour balls in warm taro, pandan, or pumpkin colors, floating in sweet coconut milk.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'khanom-chan',
              name: 'Khanom Chan',
              description:
                'A layered steamed dessert of pandan and coconut milk alternating in green and white ribbons, set firm and sliced.',
              prepTime: '20 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'khao-niao-dam',
              name: 'Khao Niao Dam',
              description:
                'Black glutinous rice cooked with coconut milk and palm sugar into a rich purple pudding served warm.',
              prepTime: '10 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'luk-chup',
              name: 'Luk Chup',
              description:
                'Miniature mung bean paste sweets hand-painted to resemble perfect tiny fruits and vegetables.',
              prepTime: '60 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 20,
            },
            {
              slug: 'coconut-ice-cream-thai',
              name: 'Coconut Ice Cream',
              description:
                'Creamy coconut milk ice cream scooped into a half coconut shell and served with roasted peanuts and corn.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'thong-yip',
              name: 'Thong Yip',
              description:
                'Pinched golden cups of sweetened egg yolk simmered in fragrant syrup—a royal Thai confection of Portuguese origin.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 16,
            },
          ],
        },
        {
          slug: 'thai-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'jok',
              name: 'Jok',
              description:
                'Thai rice porridge simmered until thick and smooth, topped with minced pork, ginger strips, and a poached egg.',
              prepTime: '5 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'pa-thong-ko',
              name: 'Pa Thong Ko',
              description:
                'Deep-fried Chinese-style dough fritters that puff into twin sticks, dipped in condensed milk or pandan custard.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'kai-jeow',
              name: 'Kai Jeow',
              description:
                'A Thai-style deep-fried omelette that puffs and crisps at the edges, seasoned with fish sauce and eaten over rice.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'khao-man-gai',
              name: 'Khao Man Gai',
              description:
                'Poached chicken over rice cooked in the same broth with garlic, served with a fermented soybean dipping sauce.',
              prepTime: '15 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'kuai-tiao-breakfast',
              name: 'Kuai Tiao',
              description:
                'Thai noodle soup of rice noodles in clear pork broth with sliced meat, fish balls, and bean sprouts.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'thai-toast',
              name: 'Khanom Pang Khai',
              description:
                'Thick-cut toast spread with kaya coconut jam and salted butter, a Bangkok café breakfast staple.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'naem-khao',
              name: 'Naem Khao',
              description:
                'Crispy rice and sour fermented pork salad rolled into lettuce cups with peanuts, lime, and fresh herbs.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'patongko-soy-milk',
              name: 'Pa Thong Ko with Soy Milk',
              description:
                'Crispy fried dough sticks served alongside warm sweetened fresh soy milk—a Sino-Thai breakfast pairing.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'sangkaya-toast',
              name: 'Sangkaya Toast',
              description:
                'Thick white bread toasted and spread with homemade pandan coconut egg jam, a Thai café morning favourite.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'roti-breakfast',
              name: 'Roti with Egg',
              description:
                'A buttery fried roti cooked with egg on the griddle, rolled and served with curry sauce or sweetened condensed milk.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 1,
            },
          ],
        },
        {
          slug: 'thai-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'cha-yen',
              name: 'Cha Yen',
              description:
                'Thai iced tea brewed strong from Ceylon tea with condensed and evaporated milk poured over a glass of crushed ice.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'oliang',
              name: 'Oliang',
              description:
                'Thai iced black coffee brewed from a robusta and grain blend, sweetened with condensed milk and poured over ice.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'nam-manao',
              name: 'Nam Manao',
              description:
                'Fresh Thai lime juice soda with salt and sugar syrup, a sharp and intensely refreshing roadside drink.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'nam-taeng-mo',
              name: 'Nam Taeng Mo',
              description:
                'Fresh watermelon juice blended smooth and poured over ice, vibrantly pink and naturally sweet.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'nam-dok-anchan',
              name: 'Nam Dok Anchan',
              description:
                'Butterfly pea flower tea that turns from deep blue to violet with a squeeze of lime, naturally caffeine-free.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'coconut-water-thai',
              name: 'Nam Maprao On',
              description:
                'Ice-cold coconut water drunk straight from a young green coconut hacked open at the street market.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'lemongrass-drink',
              name: 'Nam Takrai',
              description:
                'A chilled lemongrass and pandan infusion sweetened with palm sugar, clean and refreshingly aromatic.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'singha-beer',
              name: 'Singha Beer',
              description:
                'Thailand\'s flagship lager, a clean and slightly bitter pale ale that pairs effortlessly with spicy Thai dishes.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'mekhong-whisky',
              name: 'Mekhong Soda',
              description:
                'Thai Mekhong spirit—a blend of rice and sugarcane distillate—mixed with soda, ice, and lime wedge.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'sala-drink',
              name: 'Nam Sala',
              description:
                'A lurid pink sala fruit syrup diluted with ice water or soda, sweet and tropical with a distinctive candy aroma.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
      ],
    },
    {
      slug: 'chinese',
      name: 'Chinese Cuisine',
      country: 'China',
      flag: '🇨🇳',
      description:
        'Chinese cuisine encompasses one of the world\'s oldest and most varied culinary traditions, spanning eight major regional schools from the delicate Cantonese dim sum of Guangdong to the mouth-numbing Sichuan dishes built on doubanjiang and huajiao pepper. Across dynasties and geographies, it remains united by a mastery of wok technique, fermented ingredients, and the pursuit of harmonious flavor.',
      categories: [
        {
          slug: 'chinese-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'peking-duck',
              name: 'Peking Duck',
              description:
                'Air-dried Beijing duck lacquered in maltose syrup and roasted until the skin shatters, served with pancakes and hoisin.',
              prepTime: '30 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'kung-pao-chicken',
              name: 'Kung Pao Chicken',
              description:
                'Sichuan stir-fry of chicken, peanuts, and dried chilies tossed in a sweet, sour, and numbing huajiao sauce.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'mapo-tofu',
              name: 'Mapo Tofu',
              description:
                'Silken tofu and minced pork simmered in a fiery Sichuan sauce of doubanjiang, fermented black beans, and huajiao pepper.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'hong-shao-rou',
              name: 'Hong Shao Rou',
              description:
                'Melt-in-the-mouth pork belly red-braised in soy sauce, Shaoxing wine, and rock sugar for a lacquered, sticky result.',
              prepTime: '15 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'sweet-sour-pork',
              name: 'Sweet and Sour Pork',
              description:
                'Crispy battered pork tossed with peppers and pineapple in a vivid sweet-sour sauce, a Cantonese classic.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'char-siu',
              name: 'Char Siu',
              description:
                'Cantonese BBQ pork shoulder marinated in hoisin, soy, and honey, roasted until caramelized and red at the edges.',
              prepTime: '15 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'dan-dan-noodles',
              name: 'Dan Dan Noodles',
              description:
                'Sichuan wheat noodles in a sesame and chili oil sauce topped with crispy pork mince and preserved Yibin vegetables.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'beef-and-broccoli',
              name: 'Beef and Broccoli',
              description:
                'Velvet-textured beef and broccoli stir-fried with garlic and ginger in a rich oyster and soy sauce glaze.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'lion-head-meatballs',
              name: "Lion's Head Meatballs",
              description:
                'Large braised pork meatballs from Yangzhou slow-cooked with Napa cabbage in a light soy broth.',
              prepTime: '20 mins',
              cookTime: '1 hr',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'dim-sum',
              name: 'Dim Sum',
              description:
                'A Cantonese brunch spread of bamboo steamers holding har gow, siu mai, char siu bao, and cheung fun.',
              prepTime: '60 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 4,
            },
          ],
        },
        {
          slug: 'chinese-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'hot-sour-soup',
              name: 'Hot and Sour Soup',
              description:
                'A thick northern Chinese broth of wood ear mushrooms, tofu, bamboo shoots, and egg, sharp with vinegar and white pepper.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'wonton-soup',
              name: 'Wonton Soup',
              description:
                'Delicate pork and shrimp wontons poached and served in a clear, fragrant chicken broth with sesame oil.',
              prepTime: '30 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'egg-drop-soup',
              name: 'Egg Drop Soup',
              description:
                'A light starch-thickened chicken broth into which beaten eggs are streamed in thin ribbons, seasoned with white pepper.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'lotus-root-pork-soup',
              name: 'Lotus Root and Pork Soup',
              description:
                'Slow-simmered Cantonese tong sui of pork ribs and lotus root cooked for hours until the broth runs sweet and clear.',
              prepTime: '15 mins',
              cookTime: '2 hrs',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'winter-melon-soup',
              name: 'Winter Melon Soup',
              description:
                'Transparent cubes of winter melon and pork ribs simmered in a light broth with dried scallops and ginger.',
              prepTime: '15 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chicken-sweetcorn-soup',
              name: 'Chicken and Sweetcorn Soup',
              description:
                'Creamed corn and shredded chicken bound in a silky egg-thickened broth, a Cantonese takeaway staple worldwide.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'mongolian-hot-pot',
              name: 'Mongolian Hot Pot',
              description:
                'A divided simmering broth of clear chicken and spicy Sichuan at the table for dipping paper-thin lamb and vegetables.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'fish-head-tofu-soup',
              name: 'Fish Head Tofu Soup',
              description:
                'A milky, gelatinous broth from a pan-fried fish head braised with soft tofu, ginger, and spring onion.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'watercress-soup',
              name: 'Watercress Soup',
              description:
                'A Cantonese restorative of watercress slow-simmered with pork and dried figs to draw out a slightly bitter sweetness.',
              prepTime: '10 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'eight-treasure-congee',
              name: 'Eight Treasure Congee',
              description:
                'A festive Laba rice porridge of eight grains, legumes, and dried fruits simmered into a nourishing winter bowl.',
              prepTime: '15 mins',
              cookTime: '1 hr',
              difficulty: 'Easy',
              servings: 6,
            },
          ],
        },
        {
          slug: 'chinese-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'jianbing',
              name: 'Jianbing',
              description:
                'Beijing\'s iconic breakfast crepe of mung bean batter spread thin on a griddle with egg, crispy wonton, and hoisin.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Medium',
              servings: 1,
            },
            {
              slug: 'xiaolongbao',
              name: 'Xiaolongbao',
              description:
                'Shanghai soup dumplings pleated around pork and gelatinized broth that liquefies into a scalding mouthful when steamed.',
              prepTime: '60 mins',
              cookTime: '10 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'scallion-pancake',
              name: 'Cong You Bing',
              description:
                'Flaky multi-layered Chinese flatbreads made by rolling scallion oil into the dough and pan-frying until crispy.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'roujiamo',
              name: 'Roujiamo',
              description:
                "Xi'an's answer to the burger: slow-braised spiced pork chopped and stuffed into a baked flatbread.",
              prepTime: '20 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'tang-hulu',
              name: 'Tang Hulu',
              description:
                'Skewers of hawthorn berries or strawberries dipped in shimmering clear toffee until encased in a brittle sugar shell.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'baozi',
              name: 'Baozi',
              description:
                'Fluffy steamed buns filled with char siu pork, minced pork, or red bean paste, sold hot from bamboo steamers.',
              prepTime: '30 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'sheng-jian-bao',
              name: 'Sheng Jian Bao',
              description:
                'Pan-fried pork and soup buns from Shanghai with a crispy bottom, juicy filling, and a sesame-sprinkled top.',
              prepTime: '40 mins',
              cookTime: '15 mins',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'chuanr',
              name: 'Chuanr',
              description:
                'Cumin and chili-dusted lamb skewers grilled over charcoal, the definitive street snack of Beijing night markets.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'stinky-tofu',
              name: 'Stinky Tofu',
              description:
                'Fermented tofu deep-fried until the outside crisps while the pungent interior turns custard-soft, served with chili sauce.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'dan-bing',
              name: 'Dan Bing',
              description:
                'A Taiwanese egg crepe spread with soy paste and filled with egg, sausage, or vegetables, rolled into a warm cylinder.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
          ],
        },
        {
          slug: 'chinese-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'tangyuan',
              name: 'Tangyuan',
              description:
                'Glutinous rice balls filled with black sesame or peanut paste, served in a sweet ginger broth at the Lantern Festival.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'egg-tart',
              name: 'Egg Tart',
              description:
                'A flaky pastry shell holding a smooth, barely-set egg custard—a Cantonese bakery staple brought from Portuguese pastéis.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'mango-pudding',
              name: 'Mango Pudding',
              description:
                'Set Hong Kong-style mango jelly served chilled with evaporated milk and fresh mango slices on top.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'sesame-balls',
              name: 'Sesame Balls',
              description:
                'Chewy fried glutinous rice spheres coated in sesame seeds and filled with lotus paste or red bean—a dim sum classic.',
              prepTime: '25 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'mooncake',
              name: 'Mooncake',
              description:
                'A dense pastry filled with lotus seed paste and salted egg yolk, gifted during the Mid-Autumn Festival.',
              prepTime: '30 mins',
              cookTime: '25 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'douhua',
              name: 'Douhua',
              description:
                'Silken tofu pudding set just barely and served in warm ginger syrup or sweet peanut broth as a gentle dessert.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'nian-gao',
              name: 'Nian Gao',
              description:
                'Sticky glutinous rice cake eaten at Chinese New Year, pan-fried in egg until golden and chewy outside.',
              prepTime: '15 mins',
              cookTime: '50 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'almond-jelly',
              name: 'Almond Jelly',
              description:
                'A delicate almond-flavored milk jelly cut into cubes and served chilled in syrup with lychee and mandarin segments.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'wife-cake',
              name: 'Wife Cake',
              description:
                'A Cantonese flaky pastry with a translucent winter melon and almond paste center, thin-shelled and subtly sweet.',
              prepTime: '30 mins',
              cookTime: '20 mins',
              difficulty: 'Hard',
              servings: 8,
            },
            {
              slug: 'ba-bao-fan',
              name: 'Ba Bao Fan',
              description:
                'Eight-treasure glutinous rice pudding packed with red dates, lotus seeds, and preserved fruit, steamed in a bowl.',
              prepTime: '30 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 8,
            },
          ],
        },
        {
          slug: 'chinese-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'congee',
              name: 'Congee',
              description:
                'Smooth Cantonese rice porridge slowly cooked until silky, served with century egg, pork slices, and fried dough.',
              prepTime: '5 mins',
              cookTime: '1 hr',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'you-tiao',
              name: 'You Tiao',
              description:
                'Deep-fried leavened dough sticks puffed and slightly hollow, dipped in warm soy milk or wrapped in rice roll.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'doujiang',
              name: 'Doujiang',
              description:
                'Fresh-pressed hot soy milk served sweet or savory with vinegar and chili, paired with you tiao or shaobing.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'shaobing',
              name: 'Shaobing',
              description:
                'Sesame-encrusted baked flatbreads with flaky layers, eaten plain or stuffed with sweet paste or smoked beef.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'zhou',
              name: 'Cantonese Zhou',
              description:
                'A silky congee cooked with pork and preserved egg, finished with julienned ginger and a drizzle of sesame oil.',
              prepTime: '10 mins',
              cookTime: '1 hr',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'steamed-buns-breakfast',
              name: 'Steamed Milk Buns',
              description:
                'Cloud-soft mantou steamed buns eaten plain with condensed milk or alongside a warm bowl of soy milk.',
              prepTime: '25 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'red-bean-soup',
              name: 'Red Bean Soup',
              description:
                'Sweet azuki beans slow-simmered with dried tangerine peel and rock sugar into a warming Cantonese morning tong sui.',
              prepTime: '10 mins',
              cookTime: '1 hr',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'noodle-soup-breakfast',
              name: 'Wonton Noodle Soup',
              description:
                'Springy egg noodles in a clear shrimp-roe broth topped with plump shrimp wontons—a Hong Kong breakfast institution.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'xifan',
              name: 'Xifan',
              description:
                'Shanghai-style thin rice porridge topped with pickled mustard greens, pork floss, and a drizzle of chili oil.',
              prepTime: '5 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'jidan-bing',
              name: 'Jidan Bing',
              description:
                'A thin mung bean crepe folded with egg, scallion, and fermented black bean paste for a savory breakfast parcel.',
              prepTime: '10 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
          ],
        },
        {
          slug: 'chinese-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'pu-erh-tea',
              name: "Pu-erh Tea",
              description:
                'A deeply earthy aged fermented tea from Yunnan, compressed into cakes and brewed in small clay teapots.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'jasmine-tea',
              name: 'Jasmine Tea',
              description:
                'Green tea scented with fresh jasmine blossoms layer by layer until the leaves absorb a delicate floral fragrance.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'bubble-tea',
              name: 'Bubble Tea',
              description:
                'Taiwanese milk tea shaken over ice with chewy tapioca pearls, sipped through a fat straw—a global phenomenon.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'baijiu',
              name: 'Baijiu',
              description:
                'China\'s national spirit distilled from sorghum, fiery and complex, served neat in small ceramic cups at banquets.',
              prepTime: '2 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'shaoxing-wine',
              name: 'Shaoxing Wine',
              description:
                'Aged Chinese rice wine from Zhejiang with a nutty, amber depth used in cooking and sipped warm in winter.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'chrysanthemum-tea',
              name: 'Chrysanthemum Tea',
              description:
                'Dried chrysanthemum blossoms steeped in hot water with rock sugar for a floral, cooling herbal infusion.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'hong-kong-milk-tea',
              name: 'Hong Kong Milk Tea',
              description:
                'Coarse Ceylon tea blended through a silk stocking filter and combined with evaporated milk for a rich, smooth cup.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 2,
            },
            {
              slug: 'longjing-tea',
              name: 'Longjing Tea',
              description:
                'Dragon Well green tea from Hangzhou, flat-panned by hand to stop oxidation, with a fresh chestnut and vegetal note.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'lychee-juice',
              name: 'Lychee Juice',
              description:
                'Chilled fresh lychee blended with a little water and sugar into a fragrant, delicately sweet Asian fruit drink.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'chinese-herbal-tea',
              name: 'Chinese Herbal Tea',
              description:
                'A cooling tong sui brewed from a blend of chrysanthemum, honeysuckle, and dried fruit to clear heat and toxins.',
              prepTime: '10 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
          ],
        },
      ],
    },
    {
      slug: 'indonesian',
      name: 'Indonesian Cuisine',
      country: 'Indonesia',
      flag: '🇮🇩',
      description:
        'Indonesian cuisine reflects the extraordinary diversity of an archipelago of over 17,000 islands, weaving together Malay, Chinese, Dutch, Indian, and Arab influences into a bold, spice-forward culinary tradition. From the coconut-milk-braised rendang of West Sumatra to the layered rice ceremonial feasts of Bali, it is a cuisine that speaks through sambal, fermented shrimp paste, and the generous hand of the mortar and pestle.',
      categories: [
        {
          slug: 'indonesian-main-dishes',
          name: 'Main Dishes',
          recipes: [
            {
              slug: 'nasi-goreng',
              name: 'Nasi Goreng',
              description:
                'Indonesian fried rice stir-fried with kecap manis, shrimp paste, and chilies, crowned with a fried egg and prawn crackers.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'rendang',
              name: 'Rendang',
              description:
                'Beef slow-cooked for hours in coconut milk and a rich rempah spice paste until the sauce caramelizes dry onto the meat.',
              prepTime: '25 mins',
              cookTime: '4 hrs',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'sate-ayam',
              name: 'Sate Ayam',
              description:
                'Chicken skewers marinated in sweet soy and turmeric, grilled over charcoal and served with a peanut sauce and lontong.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'gado-gado',
              name: 'Gado-Gado',
              description:
                'Blanched vegetables, tofu, and hard-boiled egg dressed in a rich peanut and kecap manis sauce with prawn crackers.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'opor-ayam',
              name: 'Opor Ayam',
              description:
                'Chicken gently simmered in a lemongrass and galangal-spiced coconut milk broth until tender and aromatic.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'mie-goreng',
              name: 'Mie Goreng',
              description:
                'Indonesian fried egg noodles tossed with chicken, prawns, kecap manis, and sambal in a smoky wok.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'ayam-betutu',
              name: 'Ayam Betutu',
              description:
                'A Balinese whole chicken rubbed in a complex spice paste of galangal, candlenuts, and chilies and slow-roasted in banana leaf.',
              prepTime: '30 mins',
              cookTime: '3 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'bebek-goreng',
              name: 'Bebek Goreng',
              description:
                'Spice-marinated whole duck slow-cooked until tender then deep-fried until the skin shatters, a Javanese delicacy.',
              prepTime: '20 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'nasi-padang',
              name: 'Nasi Padang',
              description:
                'A Minangkabau feast of steamed rice surrounded by small plates of rendang, sambal, eggs, and curry-braised fish.',
              prepTime: '60 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'soto-ayam',
              name: 'Soto Ayam',
              description:
                'A golden turmeric chicken soup with rice vermicelli, boiled egg, and crispy shallots—Indonesia\'s beloved comfort food.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'indonesian-soups-stews',
          name: 'Soups & Stews',
          recipes: [
            {
              slug: 'rawon',
              name: 'Rawon',
              description:
                'A jet-black East Javanese beef soup colored by keluak nuts with a rich, earthy broth and bean sprout garnish.',
              prepTime: '20 mins',
              cookTime: '2 hrs',
              difficulty: 'Hard',
              servings: 4,
            },
            {
              slug: 'soto-betawi',
              name: 'Soto Betawi',
              description:
                'A rich Jakarta-style beef and offal soup simmered in coconut milk broth with tomato and fried shallots.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'sop-buntut',
              name: 'Sop Buntut',
              description:
                'Indonesian oxtail soup slow-braised with cloves, nutmeg, and celery until the meat falls from the bone.',
              prepTime: '20 mins',
              cookTime: '3 hrs',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'sayur-lodeh',
              name: 'Sayur Lodeh',
              description:
                'A Javanese mixed vegetable and tofu soup cooked in thin coconut milk with galangal and dried shrimp.',
              prepTime: '15 mins',
              cookTime: '25 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'gulai-kambing',
              name: 'Gulai Kambing',
              description:
                'Goat meat simmered in a turmeric-rich coconut curry with cinnamon, star anise, and lemongrass.',
              prepTime: '20 mins',
              cookTime: '1 hr 30 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'laksa',
              name: 'Laksa',
              description:
                'A spicy coconut milk noodle soup of Peranakan origin with shrimp, tofu puffs, and laksa leaves.',
              prepTime: '20 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'garang-asem',
              name: 'Garang Asem',
              description:
                'A Central Javanese sour chicken stew cooked in banana leaf with bilimbi fruit, chili, and galangal.',
              prepTime: '20 mins',
              cookTime: '45 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'sup-iga',
              name: 'Sup Iga',
              description:
                'Spare ribs simmered in a clear, aromatic broth spiced with nutmeg, cloves, and celery leaves.',
              prepTime: '15 mins',
              cookTime: '2 hrs',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'capcay',
              name: 'Cap Cay',
              description:
                'A Chinese-Indonesian stir-fry of mixed vegetables, quail eggs, and chicken in a light oyster sauce thickened broth.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'soto-madura',
              name: 'Soto Madura',
              description:
                'A bold Madurese beef soup with a golden broth of lemongrass, turmeric, and candlenut, served with compressed rice.',
              prepTime: '20 mins',
              cookTime: '1 hr',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'indonesian-street-food',
          name: 'Street Food',
          recipes: [
            {
              slug: 'bakso',
              name: 'Bakso',
              description:
                'Bouncy beef meatballs in a clear broth with noodles, tofu, and fried wontons, sold from push-cart vendors nationwide.',
              prepTime: '20 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'martabak',
              name: 'Martabak',
              description:
                'A thick folded pancake sold in savory versions with spiced minced beef and egg, or sweet with chocolate and cheese.',
              prepTime: '20 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'siomay',
              name: 'Siomay',
              description:
                'Steamed fish and prawn dumplings served with potato, tofu, and boiled egg, all drenched in peanut sauce.',
              prepTime: '25 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'pisang-goreng',
              name: 'Pisang Goreng',
              description:
                'Battered and deep-fried plantain bananas with a crispy golden crust and caramelized sweetness inside.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'ketoprak',
              name: 'Ketoprak',
              description:
                'Rice vermicelli, tofu, and boiled egg served with cucumber and lontong, dressed in a sweet peanut and kecap sauce.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'cireng',
              name: 'Cireng',
              description:
                'Crispy Sundanese tapioca fritters—crunchy outside and chewy within—dipped in a spicy sambal rujak sauce.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'tahu-tek',
              name: 'Tahu Tek',
              description:
                'A Surabayan dish of fried tofu, tempeh, and potato cut tableside and smothered in shrimp paste peanut sauce.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'es-campur',
              name: 'Es Campur',
              description:
                'A mixed shaved ice dessert of coconut jelly, avocado, red bean, and jackfruit in rose syrup and condensed milk.',
              prepTime: '15 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'kue-lumpur',
              name: 'Kue Lumpur',
              description:
                'Soft, steamed coconut milk and mashed potato cakes baked in rounded molds with a raisin pressed into each center.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 10,
            },
            {
              slug: 'batagor',
              name: 'Batagor',
              description:
                'Fried fish and tapioca dumplings from Bandung served with a peanut sauce, sweet soy, and sambal.',
              prepTime: '20 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 4,
            },
          ],
        },
        {
          slug: 'indonesian-desserts',
          name: 'Desserts',
          recipes: [
            {
              slug: 'klepon',
              name: 'Klepon',
              description:
                'Pandan-green glutinous rice balls filled with palm sugar that bursts when bitten, rolled in freshly grated coconut.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'kue-lapis',
              name: 'Kue Lapis',
              description:
                'A steamed layer cake of rice flour and coconut milk set in alternating pink and white stripes, slippery and sweet.',
              prepTime: '20 mins',
              cookTime: '40 mins',
              difficulty: 'Medium',
              servings: 12,
            },
            {
              slug: 'dadar-gulung',
              name: 'Dadar Gulung',
              description:
                'Pandan green crepes rolled around a filling of sweetened coconut and palm sugar cooked until fragrant.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Medium',
              servings: 8,
            },
            {
              slug: 'bika-ambon',
              name: 'Bika Ambon',
              description:
                'A porous, honeycomb-textured cake from Medan made with tapioca, coconut milk, and pandan, springy and aromatic.',
              prepTime: '20 mins',
              cookTime: '30 mins',
              difficulty: 'Hard',
              servings: 10,
            },
            {
              slug: 'es-teler',
              name: 'Es Teler',
              description:
                'A popular Indonesian dessert drink of avocado, coconut flesh, jackfruit, and sweetened condensed milk over shaved ice.',
              prepTime: '10 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'bubur-sumsum',
              name: 'Bubur Sumsum',
              description:
                'White rice flour porridge cooked smooth in coconut milk and served with liquid palm sugar poured over the top.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'serabi',
              name: 'Serabi',
              description:
                'Traditional Javanese coconut milk pancakes with crispy edges cooked in a clay mold over charcoal, topped with palm sugar.',
              prepTime: '15 mins',
              cookTime: '15 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'getuk',
              name: 'Getuk',
              description:
                'Mashed cassava sweetened with sugar and colored with pandan or pink food coloring, shaped and sprinkled with coconut.',
              prepTime: '15 mins',
              cookTime: '20 mins',
              difficulty: 'Easy',
              servings: 8,
            },
            {
              slug: 'putu',
              name: 'Putu',
              description:
                'Cylindrical steamed rice flour cakes with a palm sugar center, cooked in bamboo tubes with a whistle of steam.',
              prepTime: '15 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 6,
            },
            {
              slug: 'wajik',
              name: 'Wajik',
              description:
                'Diamond-shaped glutinous rice cakes cooked with palm sugar, coconut milk, and pandan until sticky and shiny.',
              prepTime: '10 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 12,
            },
          ],
        },
        {
          slug: 'indonesian-breakfast',
          name: 'Breakfast',
          recipes: [
            {
              slug: 'bubur-ayam',
              name: 'Bubur Ayam',
              description:
                'Indonesian chicken congee topped with shredded chicken, fried shallots, celery, and ginger soy sauce.',
              prepTime: '10 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'nasi-uduk',
              name: 'Nasi Uduk',
              description:
                'Fragrant coconut milk rice cooked with lemongrass and bay leaf, served with fried chicken, tempe, and sambal.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'lontong-sayur',
              name: 'Lontong Sayur',
              description:
                'Compressed rice cakes in a rich coconut milk vegetable curry of chayote and boiled egg.',
              prepTime: '20 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'nasi-kuning',
              name: 'Nasi Kuning',
              description:
                'Yellow turmeric and coconut rice molded into a cone and served with assorted side dishes for celebrations.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Easy',
              servings: 6,
            },
            {
              slug: 'roti-bakar',
              name: 'Roti Bakar',
              description:
                'Indonesian toast griddled in butter and spread with kaya jam, chocolate sprinkles, or cheese and condensed milk.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'tahu-tempe-bacem',
              name: 'Tahu Tempe Bacem',
              description:
                'Tofu and tempeh slow-braised in sweet soy sauce, palm sugar, and galangal until they absorb a caramel glaze.',
              prepTime: '10 mins',
              cookTime: '40 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'ketan-hitam',
              name: 'Ketan Hitam',
              description:
                'Black glutinous rice porridge sweetened with palm sugar and finished with salted coconut cream.',
              prepTime: '5 mins',
              cookTime: '45 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'nasi-lemak-indonesian',
              name: 'Nasi Lemak',
              description:
                'Coconut milk rice wrapped in banana leaf and served with sambal, fried anchovies, cucumber, and boiled egg.',
              prepTime: '15 mins',
              cookTime: '30 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'ketupat-opor',
              name: 'Ketupat Opor',
              description:
                'Woven palm-leaf rice dumplings served with opor ayam coconut curry, a traditional Lebaran morning feast.',
              prepTime: '30 mins',
              cookTime: '1 hr',
              difficulty: 'Hard',
              servings: 6,
            },
            {
              slug: 'seblak',
              name: 'Seblak',
              description:
                'Wet crackers cooked in a spicy kencur and chili broth with egg and vegetables, a bold Sundanese morning staple.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 2,
            },
          ],
        },
        {
          slug: 'indonesian-drinks',
          name: 'Drinks',
          recipes: [
            {
              slug: 'es-teh-manis',
              name: 'Es Teh Manis',
              description:
                'Ice-cold sweet black tea poured over a tall glass of ice—Indonesia\'s most ordered drink at any warung.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'jamu',
              name: 'Jamu',
              description:
                'A traditional Javanese herbal tonic of turmeric, ginger, tamarind, and palm sugar drunk fresh each morning.',
              prepTime: '10 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'es-jeruk',
              name: 'Es Jeruk',
              description:
                'Freshly squeezed Indonesian citrus juice mixed with water, sugar, and ice for a bright and tangy refresher.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 2,
            },
            {
              slug: 'wedang-jahe',
              name: 'Wedang Jahe',
              description:
                'A warming Javanese ginger tea of bruised fresh ginger simmered with palm sugar and pandan leaves.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'bajigur',
              name: 'Bajigur',
              description:
                'A Sundanese hot coconut milk and palm sugar drink spiced with ginger and pandan, served in clay cups.',
              prepTime: '5 mins',
              cookTime: '10 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'es-dawet',
              name: 'Es Dawet',
              description:
                'Pandan jelly noodles in a sweet coconut milk and palm sugar broth poured over crushed ice.',
              prepTime: '20 mins',
              cookTime: '10 mins',
              difficulty: 'Medium',
              servings: 4,
            },
            {
              slug: 'bandrek',
              name: 'Bandrek',
              description:
                'A warming West Javanese spiced drink of coconut milk, ginger, lemongrass, and cloves against the highland chill.',
              prepTime: '5 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'kopi-tubruk',
              name: 'Kopi Tubruk',
              description:
                'Indonesian-style coffee brewed by pouring boiling water directly over coarsely ground beans in the cup.',
              prepTime: '5 mins',
              cookTime: '5 mins',
              difficulty: 'Easy',
              servings: 1,
            },
            {
              slug: 'bir-pletok',
              name: 'Bir Pletok',
              description:
                'A non-alcoholic Betawi heritage drink of ginger, rose syrup, and spices that turns ruby red when shaken.',
              prepTime: '10 mins',
              cookTime: '15 mins',
              difficulty: 'Easy',
              servings: 4,
            },
            {
              slug: 'es-kelapa-muda',
              name: 'Es Kelapa Muda',
              description:
                'Young green coconut water poured over ice with scoops of the tender jelly flesh, refreshing and replenishing.',
              prepTime: '5 mins',
              cookTime: '0 mins',
              difficulty: 'Easy',
              servings: 1,
            },
          ],
        },
      ],
    },
  ],
};

fs.mkdirSync(path.join(__dirname, '..', 'data'), { recursive: true });
fs.writeFileSync(OUTPUT_PATH, JSON.stringify(taxonomy, null, 2), 'utf-8');

const totalRecipes = taxonomy.cuisines.reduce(
  (sum, cuisine) =>
    sum +
    cuisine.categories.reduce((s, cat) => s + cat.recipes.length, 0),
  0
);
console.log(`✓ Wrote ${OUTPUT_PATH}`);
console.log(
  `  ${taxonomy.cuisines.length} cuisines · ${taxonomy.cuisines[0].categories.length * taxonomy.cuisines.length} categories · ${totalRecipes} recipes`
);
